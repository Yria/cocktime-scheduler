...
try {
	...
	const manage = async action => query("select dues_v2_manage($1,$2,'offline dry run',$3)", [action, (await mode()).revision, randomUUID()]);
	...
	const baseline = await query("select dues_v2_snapshot()");
	...
	await manage("undo");
	assert.deepEqual(await query("select dues_v2_snapshot()"), baseline, "Undo did not restore the imported facts");
	...
	assert.equal(await legacyHash(), originalHash, "Rollback edited a legacy fact");
	console.log(JSON.stringify({ observed_at: source.observed_at, ...amounts, legacy_hash_unchanged: true, replace_and_undo: "passed", july_august_september_cash_totals: "passed", rollback: "passed" }, null, 2));
} finally {
	await db.close();
}
