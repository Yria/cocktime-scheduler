...
`node supabase/tests/resize_courts.test.mjs`는 실제 코트 축소 트리거와 기존 동기화 트리거를
로컬 PGlite에서 실행한다. 중간/여러 빈 코트 삭제, 진행 경기의 연속 번호와 명단 보존,
편성·예약의 예비 그룹 전환, 완료 경기 기록 보존, 재증설, 빈 코트 부족 시 전체 롤백,
조회 기기의 코트 갱신 버전을 검사한다. 운영 DB 쓰기나 다중 연결 잠금 경합은 실행하지 않는다.
...
최신 마이그레이션은 전원 출전 확인을 해당 경기·명단에만 허용하고, 미확인·일반회원·편집권 미보유 요청은 계속 거부하는지 검증한다.
...
`20260918010000_private_match_proposals.sql`을 두 번 적용한 뒤 실제 RPC와 SELECT RLS를 실행한다.
작성자·운영진만 조회 가능, 제안에 포함된 다른 회원 조회 불가, 비참가자 전송 차단,
직접 테이블 쓰기 차단, 중복 없는 1~4명, 같은 세션 검증, 재시도 멱등성,
작성자 취소·운영진 확인 권한, 종료 세션 차단과 공유 보드 불변을 확인한다.
실제 Supabase Realtime 네트워크 전달과 다중 연결 잠금 경합은 이 검사 범위에 포함되지 않는다.
