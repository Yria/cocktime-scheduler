-- Complete TODO refund sender matching; additive follow-up to the installed V2 schema.
...
-- A reference roster is read only; choosing it never generates or modifies dues.
create function public.dues_v2_reference_members(p_session_id bigint,p_meal boolean default false) returns jsonb
...
begin
  ...
  return (select coalesce(jsonb_agg(member_id order by member_id),'[]') from (
    select a.member_id from public.attendances a where a.session_id=p_session_id and a.status<>'cancelled'
      and (not p_meal or a.meal_joining is not false)
    union select sp.member_id from public.session_players sp where sp.session_id=p_session_id and sp.member_id is not null and not p_meal
  ) roster);
end $$;
...
-- Manual reference rosters also include sessions without court charges.
create or replace function public.dues_v2_sessions() returns jsonb language plpgsql stable security definer set search_path='' as $$
begin
  if public.is_admin() is not true then raise exception 'forbidden' using errcode='42501'; end if;
  return (select coalesce(jsonb_agg(jsonb_build_object('id',s.id,'title',s.title,'scheduled_at',s.scheduled_at,'ends_at',s.ends_at,'place',p.name,'court',coalesce(p.charges_court_fee,false))
    order by s.scheduled_at desc,s.id),'[]') from public.sessions s left join public.places p on p.id=s.place_id
    where s.scheduled_at is not null and s.status in ('open','active','closed'));
end $$;


-- One label contract for admin selectors, member statements and confirmation summaries.
create function public.dues_v2_session_label(p_id bigint) returns text
language plpgsql stable security definer set search_path='' as $$
declare s record; label text; peers int; simultaneous int; namesakes int;
begin
  select t.*,p.name as place from public.sessions t left join public.places p on p.id=t.place_id where t.id=p_id into s;
  if not found or s.scheduled_at is null then return '회차 #'||p_id; end if;
  select count(*), count(*) filter(where t.scheduled_at=s.scheduled_at and t.ends_at is not distinct from s.ends_at),
    count(*) filter(where t.scheduled_at=s.scheduled_at and t.ends_at is not distinct from s.ends_at and t.title is not distinct from s.title)
    into peers,simultaneous,namesakes from public.sessions t left join public.places p on p.id=t.place_id
    where t.status in ('open','active','closed') and (t.scheduled_at at time zone 'Asia/Seoul')::date=(s.scheduled_at at time zone 'Asia/Seoul')::date
      and p.name is not distinct from s.place;
  label:=to_char(s.scheduled_at at time zone 'Asia/Seoul','YYYY-MM-DD')||' · '||coalesce(nullif(s.place,''),nullif(s.title,''),'장소 미정');
  if peers>1 then
    label:=label||' · '||to_char(s.scheduled_at at time zone 'Asia/Seoul','HH24:MI')||case when s.ends_at is null then '' else '–'||to_char(s.ends_at at time zone 'Asia/Seoul','HH24:MI') end;
    if simultaneous>1 then
      if nullif(s.title,'') is not null then label:=label||' · '||s.title; end if;
      if namesakes>1 then label:=label||' · #'||s.id; end if;
    end if;
  end if;
  return label;
end $$;
revoke all on function public.dues_v2_session_label(bigint) from public,anon,authenticated;
