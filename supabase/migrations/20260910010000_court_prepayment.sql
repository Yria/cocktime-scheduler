-- Restore upcoming court prepayment within the existing atomic pay command.
-- Installation changes no bank transactions, charges, allocations or balances.
set local lock_timeout = '5s';

-- All open fee-collecting sessions, independent of the viewed/deposit month.
-- Reuse the existing billing rule and only expose confirmed/late-pool attendance.
create function public.dues_v2_prepayment_candidates() returns jsonb
language plpgsql stable security definer set search_path='' as $$
declare s record; candidate jsonb; line jsonb; result jsonb:='[]'; label text;
begin
  for s in select ss.id,ss.scheduled_at from public.sessions ss
    join public.places pl on pl.id=ss.place_id
    left join public.recurring_schedules r on r.id=ss.recurring_schedule_id
    where ss.status='open' and ss.scheduled_at is not null and pl.charges_court_fee
      and (coalesce(ss.court_fee,r.court_fee) is null or coalesce(ss.court_fee,r.court_fee)>0)
    order by ss.scheduled_at,ss.id
  loop
    candidate:=public.dues_v2_candidates_internal('court',to_char(s.scheduled_at at time zone 'Asia/Seoul','YYYY-MM'),s.id);
    label:=public.dues_v2_session_label(s.id);
    for line in select value from jsonb_array_elements(candidate->'payload'->'lines') loop
      if (line->>'amount')::integer>0 and exists(select 1 from public.attendances a
        where a.session_id=s.id and a.member_id=(line->>'member_id')::uuid and a.status in ('confirmed','late_pool')) then
        result:=result||jsonb_build_array(jsonb_build_object('session_id',s.id,'member_id',line->>'member_id',
          'amount',(line->>'amount')::integer,'due_ym',line->>'due_ym','label',label,
          'occurred_on',candidate->'payload'->>'date'));
      end if;
    end loop;
  end loop;
  return result;
end $$;
revoke all on function public.dues_v2_prepayment_candidates() from public,anon,authenticated;

-- Called after the command has locked the financial revision and matched the payer.
-- Issue only this member's charge, then allocate the selected deposit in the same transaction.
create function public.dues_v2_prepay_due(p_session bigint,p_member uuid,p_expected integer) returns uuid
language plpgsql security definer set search_path='' as $$
declare day date; candidate jsonb; line jsonb; issued jsonb; result uuid;
begin
  select (s.scheduled_at at time zone 'Asia/Seoul')::date into day
    ...
    where s.id=p_session and s.status='open' and s.scheduled_at is not null and pl.charges_court_fee
      and (coalesce(s.court_fee,r.court_fee) is null or coalesce(s.court_fee,r.court_fee)>0)
      and exists(select 1 from public.attendances a where a.session_id=s.id and a.member_id=p_member
        and a.status in ('confirmed','late_pool'));
  if day is null then raise exception '선납할 회차 또는 참석 상태가 변경됐습니다. 다시 선택하세요' using errcode='40001'; end if;
  candidate:=public.dues_v2_candidates_internal('court',to_char(day,'YYYY-MM'),p_session)->'payload';
  select value into line from jsonb_array_elements(candidate->'lines') where value->>'member_id'=p_member::text;
  if line is null or p_expected is null or p_expected<=0 or (line->>'amount')::integer<>p_expected then
    raise exception '선납 대상 또는 금액이 변경됐습니다. 다시 선택하세요' using errcode='40001';
  end if;
  issued:=public.dues_v2_issue(candidate||jsonb_build_object('lines',jsonb_build_array(line),
    'basis',(candidate->'basis')||jsonb_build_object('prepayment',true)));
  select d.id into strict result from public.dues_v2_due d where d.charge_id=(issued->'charges'->>0)::uuid;
  return result;
end $$;
revoke all on function public.dues_v2_prepay_due(bigint,uuid,integer) from public,anon,authenticated;

create or replace function public.dues_v2_execute(p jsonb,p_request uuid,p_revision bigint) returns jsonb
language plpgsql security definer set search_path='' as $$
#variable_conflict use_column
<<mutation>>
declare ctl public.dues_v2_control; prior public.dues_v2_operations; outstanding_before bigint; member_balance_before bigint;
...
begin
  ...
  select coalesce(sum(remaining),0) into outstanding_before from public.dues_v2_due;
  select coalesce(sum(amount),0) into member_balance_before from public.dues_v2_positions where purpose in ('member_pending','carry');
  ...
  if action='issue' then
    ...
    for line in select value from jsonb_array_elements(p->'lines') loop
      ...
      if line->>'session_id' is not null then
        if line->>'due_id' is not null then raise exception '납부할 부과 또는 선납 회차 하나만 선택하세요'; end if;
        due_id:=public.dues_v2_prepay_due((line->>'session_id')::bigint,(p->>'owner_id')::uuid,(line->>'expected_amount')::integer);
      else
        due_id:=(line->>'due_id')::uuid;
      end if;
      perform public.dues_v2_allocate((line->>'position_id')::uuid,due_id,(line->>'amount')::integer,coalesce((p->>'proxy')::boolean,false));
  ...
  result:=result||jsonb_build_object('auto_applied',applied,
    ...
    'member_balance_before',member_balance_before,
  ...
  insert into public.dues_v2_operations(epoch,request_id,action,payload,actor_id,result)
    values(ctl.epoch,p_request,action,p,public.current_member_id(),result) returning id into op_id;
...
create or replace function public.dues_v2_read() returns jsonb language plpgsql stable security definer set search_path='' as $$
...
begin
  ...
  select jsonb_build_object(
    ...
    'prepayments',case when admin then public.dues_v2_prepayment_candidates() else '[]'::jsonb end,
    'groups',(select coalesce(jsonb_agg(to_jsonb(g)||case when g.kind='court' and g.session_id is not null then jsonb_build_object('label',public.dues_v2_session_label(g.session_id)) else '{}'::jsonb end||jsonb_build_object('basis',case when admin then g.basis else '{}'::jsonb end)),'[]') from public.dues_v2_groups g),
...
revoke all on function public.dues_v2_execute(jsonb,uuid,bigint) from public,anon,authenticated;
-- Existing clients reload the shared read model on their next mode poll.
update public.dues_v2_control set revision=revision+1 where id=1;
