-- Retire the completed migration's recovery controls and recovery-only data.
-- Keep financial facts, request IDs/results, and reverted_at (old requests must not replay).
-- Current deployments must be active and unpaused. A fresh database imports once.
set local lock_timeout = '5s';
do $$
declare ctl public.dues_v2_control;
begin
  select * into strict ctl from public.dues_v2_control where id=1 for update;
  if ctl.paused then raise exception '회계 처리가 중지된 상태에서는 운영복구를 제거할 수 없습니다'; end if;
  if not ctl.enabled then
    if ctl.activated_at is not null then raise exception '기존 부과로 복귀한 상태에서는 운영복구를 제거할 수 없습니다'; end if;
    perform public.dues_v2_import();
    update public.dues_v2_control set enabled=true,activated_at=now() where id=1;
  end if;
end $$;

-- Confirmation/preview still share the existing atomic validator, revision lock,
-- idempotency journal, payer checks and financial conservation checks.
-- Only the two summary totals are needed; no full ledger copies or recovery deltas.
...
-- Metadata for financial operations remains available in the processing history.
-- Migration control actions and their restore data are no longer used.
drop function public.dues_v2_manage(text,bigint,text,uuid);
drop function public.dues_v2_export();
drop function public.dues_v2_import();
drop function public.dues_v2_restore(jsonb);
drop function public.dues_v2_restore_delta(jsonb,jsonb);
drop function public.dues_v2_delta(jsonb,jsonb);
drop function public.dues_v2_snapshot();

delete from public.dues_v2_operations where action in ('activate','pause','resume','undo','rollback');
alter table public.dues_v2_control drop column baseline;
alter table public.dues_v2_operations drop column before_state, drop column after_state;
-- Notify existing clients once; preserve epoch so prior request IDs remain idempotent.
update public.dues_v2_control set revision=revision+1 where id=1;
