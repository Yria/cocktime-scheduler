-- Inline settlement: matching an unassigned deposit and paying/carrying it are
-- one existing command, one atomic journal entry, and one undo. No data backfill.
create function public.dues_v2_match_position(p_position uuid,p_owner uuid,p_confirm boolean) returns void
language plpgsql security definer set search_path='' as $$
declare pos public.dues_v2_positions;
begin
  ...
  if exists(select 1 from (
    select owner_id from public.dues_v2_positions where bank_tx_id=pos.bank_tx_id
    union all select owner_id from public.dues_v2_allocations where bank_tx_id=pos.bank_tx_id
    union all select owner_id from public.dues_v2_refunds where in_tx_id=pos.bank_tx_id
  ...
  update public.dues_v2_refunds set owner_id=p_owner where in_tx_id=pos.bank_tx_id and owner_id is null;
end $$;
revoke all on function public.dues_v2_match_position(uuid,uuid,boolean) from public,anon,authenticated;
  ...
  if action='issue' then
    ...
    for line in select value from jsonb_array_elements(p->'lines') loop
      if p->>'owner_id' is not null then
        perform public.dues_v2_match_position((line->>'position_id')::uuid,(p->>'owner_id')::uuid,coalesce((p->>'confirm_owner')::boolean,false));
      end if;
