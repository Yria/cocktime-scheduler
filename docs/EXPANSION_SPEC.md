...
| RPC | 시그니처 | 권한 | 동작 |
|-----|---------|------|------|
| `join_session` | `(p_session_id bigint) → attendances` | 로그인 회원 | 정원 여유면 confirmed. 만석이면 **부과 없는 일정에서만 프리패스 두 갈래**(①운영진: 확정 운영진 총수 < max(2, floor(정원/8)) ②신규회원: 가입 2주 이내, **상한 없음** · §5.1)로 정원 초과 confirmed, 그 외 waitlisted. 신규 프리패스가 **부여되는 지점은 이 함수와 `set_late_minutes` 뿐**이다. **비활성 회원 차단**(`member inactive`, 20260821020000 — 이 함수를 재정의할 때 반드시 함께 옮긴다). 중복신청 차단, 취소후재신청은 같은 행 갱신. 참여 가능 = `status='open'` + **종료 상한(`ends_at<=now()`→`session ended`)** 가드 |
...
| `set_session_capacity` | `(p_session_id bigint, p_capacity int) → jsonb{promoted,demoted}` | 운영진 | 정원 UPDATE+재조정 원자 RPC. open 세션만 재조정: **position ASC 단일 패스 그리디**로 전원을 다시 배분한다 — 앞에서부터 정원을 채우고(게스트는 확정 2명 미만일 때만), 만석 이후엔 운영진 총수 < max(2, floor(정원/8))일 때 운영진만 초과 confirmed, 나머지는 waitlisted. 이미 확정된 정원 외 자리(신규·우선참여권)는 그대로 보존한다. 승격도 강등도 그 한 패스의 결과다(정원↓ 시 초과분 강등이 따로 있는 게 아니다). position 은 보존한다. 신규 유예를 **세션 날짜**로 판정하므로 며칠 뒤 다시 돌려도 같은 답이 나온다(멱등). 알림 대상 `coalesce(invited_by, member_id)`(게스트면 payload.guest_name). 승격 `promoted`/강등 `demoted`. 잠금 sessions→session_counters→attendances. open 이 아닌 세션도 **카운터는 실제값으로 정합**(20260806010000 — 종료 세션 드리프트 잔존 방지) |
...
- **운영진 프리패스(부과 없는 일정)**: 만석이어도 **확정 운영진 총수 < max(2, floor(정원/8))** 일 때 대기 중 운영진 1명을 정원 초과 확정. **정원 안에 이미 들어와 있는 운영진도 기준 인원에 포함**된다. 기준은 최소 2명이고, 정원 24·28명은 3명, 32명은 4명이다(2026-09-21, 마이그레이션 `20260921000000_scale_operator_freepass.sql`). 계산은 `operator_freepass_limit(capacity)`로 통일하며, 신청·대기 승급·늦참 복귀·정원 변경·우선참여권 안내에 적용한다. 정원 18 기준: ①회원16+운영진2=18 만석 → 3번째 운영진 대기 ②거기서 운영진 1명 취소 → 대기 1순위가 누구든 참여 ③회원17+운영진1=18 만석에서 운영진 참여 → 초과 확정 19명 ④거기서 회원 1명 취소 → 회원16+운영진2=18 → 아무도 승격 안 됨.
- **신규회원 2주 프리패스(부과 없는 일정)**: 만석이어도 **가입 2주 이내 회원**은 대기에 걸리지 않고 확정된다(2026-09-03, 마이그레이션 `20260903000000` → `20260903010000`).
  ...
  - **운영진 프리패스의 정원 카운트 방식은 유지한다** — 종전처럼 정원 카운트에 들어가고 '확정 운영진 총수 < max(2, floor(정원/8))' 로 판정한다. 두 프리패스의 성격이 갈리는 것은 의도다: 운영진은 정원을 함께 쓰는 구성원이고, 신규 유예는 정원과 무관한 한시적 예외다.
  ...
  - 운영진 프리패스는 '확정 운영진 총수 < max(2, floor(정원/8))' 기준을 쓴다(신규회원 예외와 별개 — 20260921000000).
  - 운영진 캡과 기준이 다른 건 의도다. 운영진은 '총수'라 정원 안 운영진도 기준 인원에 포함되지만, 신규는 '초과분'만 센다 — 정원 안 신규를 포함시키면 일찍 신청한 신규가 늦게 가입한 신규의 자리를 잡아먹기 때문이다.
  ...
  - 승격 루프 종료성: 매 호출마다 확정 운영진 총수를 다시 세므로 정원별 기준 인원에 도달하면 운영진 분기가 닫힌다. 정원 분기는 count=capacity에서 멈추며, 신규회원 프리패스는 승격 루프에 없다.
