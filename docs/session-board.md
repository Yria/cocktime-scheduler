...
> 팀매칭의 사용자 규칙·자동편성·확정 순번·회원 파티는 [팀매칭 기획](TEAM_MATCHING_SPEC.md)에 통합했다. 이 문서는 보드 구조·인터랙션 상세를 다루며, [정합성 검토](TEAM_MATCHING_REVIEW.md)에 현재 코드와의 차이를 기록했다(2026-09-07).
...
- **회원 파티(2026-09-13 임시 비활성화)**: 재개 시 참가 운영진이 모두 경기중일 때 회원들이 가운데 버튼을 누르고 있어 4인 확정 팀을 만든다. 선발은 현재 홀드한 가용 회원 안에서 기존 추천 규칙을 따른다(§13).
...
- **공유 배치 복원**: 편집자의 코트·그룹·자유 자석 좌표를 명단과 함께 `sessions.board_drafts.layout`에 저장한다. 드래그 프레임은 저장하지 않고 드롭·정렬 결과만 기존 버전 CAS로 저장한다. 저장된 좌표가 있으면 편집자와 뷰어 모두 자동 정렬을 멈추고 그대로 복원한다. 화면 크기가 다르면 좌표 대신 배율만 조정한다. 좌표 없는 구버전 데이터만 기존 자동 정렬을 사용한다. 화면 이탈 시 마지막 저장을 마친 뒤 편집 락을 해제하며, 다른 세션으로 이동한 뒤 도착한 응답은 새 세션에 적용하지 않는다.
...
│  ├─ memberParty.ts       — 홀드 참가 회원 중 4명 선발 + 2v2 페어 편성 (순수)
...
> **2026-09-18 사용 종료.** 함께 누르는 방식은 회원 매칭 제안으로 대체한다. `SessionBoard.tsx`에서 진입점과 기능 플래그를 제거했으며 아래 내용은 과거 설계 기록이다. 기존 서버 RPC와 파일은 이번 변경에서 삭제하지 않았다.

운영진이 경기하는 동안 대기 회원들이 다음 4인 팀을 만들 수 있다. 별도 세션 설정 없이 **해당 세션에 참가한 운영진이 1명 이상이고 모두 경기중**일 때 자동으로 나타난다. 해당 상황이 아니면 가운데 버튼, 안내와 장식을 모두 숨긴다.

- **참가**: 회원이 가운데 버튼을 누르고 있는 동안 참여한다. 첫 홀드에서 세션 전체가 공유하는 **5초 카운트다운**이 시작되고, 다른 회원도 같은 마감까지 참가할 수 있다. 손을 떼면 즉시 참가에서 빠진다.
- **선발 시점**: 마감 시점에 여전히 누르고 있는 회원만 후보가 된다. 첫 참가자나 요청자에게 고정 자리를 주지 않는다. 가용 후보가 **4명 미만이면 취소**하며, 4명 초과이면 기존 추천 알고리즘으로 정확히 4명을 선발한다.
- **가용성**: `selectMemberParty`가 일반 추천 풀을 홀드 회원 집합으로 제한한다. 경기중(코트 배치 또는 선수 상태), 휴식, 다른 팀 anchor·ghost 예약, 자석 없는 선수, 회원 연결 없는 게스트, 콕 체크 ON일 때 콕 미확인 선수는 제외한다. 버튼을 누르지 않은 대기자나 경기중 선수로 충원하지 않는다.
- **매칭 규칙**: `autoFillTeammates`의 경기수·대기시간·재결성 회피·실력·성별·로테이션 점수를 그대로 쓰되 경기중 허용 상한은 **0명**이다. 4명 선발 후 `pairPlayers`로 2v2 균형을 맞춘다. 자세한 가중치는 [팀 추천/편성 알고리즘 규칙](./TEAM_GENERATION_RULES.md#회원-파티--selectmemberparty)을 따른다.
- **서버 확정**: 서버가 참가 운영진 경기 상태, 현재 홀드, 4명의 가용성, 기존 보드 소속을 다시 확인하고 **원자적으로 매칭확정 팀을 생성**한다. 동시에 마감 요청이 들어와도 동일 라운드에서 중복 팀이 생기지 않아야 한다.
- **읽기 모드 전용**: 현재 편집 권한을 가진 운영진에게는 기능을 숨기고 참가·확정을 차단한다. 읽기 모드인 회원만 참여하며, 누르는 도중 편집 권한을 가져오면 즉시 해제한다. 서버도 누른 기기의 `board_client_id`를 현재 `editor_client_id`와 비교해 편집 기기의 참여를 제외한다(lease 만료와 무관한 기존 sticky 소유 기준).
- **권한**: 이 기능은 일반 회원의 보드 편집·경기시작 권한을 확대하지 않는다. 파티는 확정 팀 대기 순서에 합류하고 이후 경기시작은 기존 운영진 권한을 따른다.
- **디자인**: 가운데 게임 형태의 참가 버튼에 누르고 있는 상태와 남은 시간을 표시한다. 실제 누른 회원만 주변에 하나씩 떠오르며, 빈 참가 대기 이미지·궤도 장식·닫기 버튼은 표시하지 않는다. 참가 인원·진행 결과·취소 사유를 함께 안내한다.
- **완료 화면**: 선정된 4명과 파티 완성 상태를 표시하고, 하단 참여 조건·도움말·완료 안내는 숨긴다. 아직 팀 없는 회원은 다음 파티에 다시 참여할 수 있으며, 이미 편성된 회원을 다음 모집에 중복 참여시키지 않는다.
- **실시간 전파**: 첫 참여·추가 참여·해제·확정·취소가 DB에 반영되면 트리거가 기존 `session-bc:<세션 ID>` 채널에 `member_party_changed` Broadcast를 보낸다. 수신 화면은 약 100ms 동안 연속 알림을 합쳐 인증된 상태 RPC를 조회하고 참가 인원·공유 마감·결과를 반영한다. 공개 채널에는 빈 변경 알림만 보내며 회원 정보나 클라이언트가 보낸 참가 명단을 신뢰하지 않는다.
- **누름 확인과 복구**: 누르는 기기는 약 1초마다 RPC로 3초 lease를 연장한다. 유효한 lease 연장은 추가 Broadcast를 발생시키지 않는다. 손을 떼면 본인 화면에서 즉시 빠지고 해제 RPC 이후 다른 화면에도 전파된다. 재접속·화면 복귀 시 즉시 재조회한다. Realtime 연결 중에는 모집 관찰자가 3초, 대기 화면이 25초 간격으로 누락·lease 만료를 보정하며 마감 시각에도 조회한다. Realtime 미연결 또는 마이그레이션 미적용 서버에서는 기존 모집 중 1초/대기 3초 조회를 유지한다. 확정된 팀은 기존 보드 동기화 경로로 전파한다.

## 14. 회원 매칭 제안

일반 회원에게는 운영진용 보기 전용 프레임·권한 버튼을 표시하지 않는다. 세션에 참여한 회원은 대기 중인 자유 자석을 다른 자유 자석에 겹쳐 놓아 2명으로 매칭 제안을 시작한다. 운영진의 새 그룹과 같은 거리·중심점 규칙을 사용한다. 빈 공간에 놓으면 위치만 이동하고 그룹은 만들지 않는다. 실제 팀·코트의 빈 칸에 놓아도 제안은 만들지 않는다. 제안에 최대 4명까지 추가할 수 있고, 묶음 밖으로 빼거나 더블탭하면 선택이 해제된다. 실제 팀 소속·예약·경기 중인 회원을 제안으로 복제할 수 없으며, 휴식·콕 미확인 회원도 새 제안에 넣지 않는다.

- 전송 전 묶음은 `matchProposalStore`의 메모리에만 존재한다. 공유 `boardStore.drafts`·예약·경기 상태와 별개이며 새로고침, 보드 이탈, 계정·세션 변경 시 사라진다.
- 묶음 아래 경기시작과 같은 형태의 **매칭 제안** 버튼을 누를 때만 서버에 저장한다. 전송 중에는 명단 편집과 중복 전송을 막고, 실패하면 묶음을 유지한다. 같은 요청 ID를 재시도해도 중복 제안이 생기지 않는다.
- 보낸 제안은 **작성자와 운영진 보드에 점선 테두리 그룹**으로 표시한다. 작성자 화면에서는 전송 전 위치를 유지한다. 별도 목록·상단 제안 버튼은 없다. 작성자는 그룹 아래에서 취소할 수 있고, 운영진은 인원을 수정하거나 **X로 거절**할 수 있다. 취소·거절은 양쪽 화면에서 사라지며 종료된 제안을 늦은 확인 요청으로 되살릴 수 없다. 제안은 실제 팀을 자동으로 만들거나 출전 순서를 예약하지 않는다.
- 제안 그룹은 각 화면에서 이동할 수 있고, **정렬** 버튼으로 작성 중 묶음·보낸 제안을 실제 팀과 함께 격자 배치한다. 제안의 위치는 서버나 다른 사람의 배치에 반영하지 않는다. 전송된 명단은 그룹을 드래그해도 바뀌지 않는다.
- 조회 경계는 화면 필터가 아니라 `match_proposals`의 SELECT RLS다. 작성자와 운영진만 읽으며, 명단에 포함된 다른 회원도 볼 수 없다. INSERT/UPDATE는 검증된 RPC만 허용한다. 일반 회원인 작성자는 현재 세션 참가자여야 하며, 운영진은 참가 등록 없이도 보기 모드에서 제안할 수 있다. 명단은 같은 세션의 중복 없는 1~4명이어야 한다.
- 별도 Postgres Changes INSERT/UPDATE 구독을 사용한다. 공개 세션 Broadcast에 제안 ID나 명단을 보내지 않는다. 구독 연결 완료·화면 복귀·온라인 복귀와 30초 간격 조회로 상태를 복구한다. 숨겨진 탭은 주기 조회하지 않는다. 늦게 끝난 조회가 방금 전송·취소한 상태를 덮지 않도록 쓰기 리비전을 비교한다.
- 운영진 경기 여부와 무관하게 제안할 수 있다. 운영진의 기존 편집권과 경기 완료 권한은 유지한다.

운영진 보기 모드에서는 일반 참가자와 같은 자석 묶기·제안 전송·본인 제안 취소를 제공한다. 실제 코트 편성과 경기 시작은 편집 권한이 있어야 한다. 참가하지 않은 운영진까지 제안을 허용하는 서버 변경은 `supabase/migrations/20260922020000_admin_viewer_match_proposals.sql`에 있다.

배포 시 `supabase/migrations/20260918010000_private_match_proposals.sql`과 `20260918020000_reject_match_proposals.sql`, `20260918030000_edit_and_start_match_proposals.sql`을 프런트 배포 전에 적용해야 한다. 새 테이블·RLS·RPC와 Realtime publication 등록, 운영진 거절 권한을 포함한다. 취소·거절은 행 삭제가 아닌 상태 변경으로 전달한다. 이는 [Supabase Postgres Changes 문서](https://supabase.com/docs/guides/realtime/postgres-changes)의 DELETE 이벤트 RLS 제약을 피하기 위한 구성이다.

검증: `node supabase/tests/match_proposals.test.mjs`는 로컬 PGlite에서 실제 SQL과 역할별 RLS, 1~4명 검증, 재시도, 취소·거절 권한, 인원 편집 동시성, 직접 경기시작, 공유 보드 불변을 확인한다. `e2e/match-proposals.spec.ts`는 외부 요청을 가로채 실제 보드 드래그·전송·실패 보존·역할별 화면과 모바일/데스크톱 캡처를 확인한다. 실제 운영 Supabase의 Realtime 전달은 이 로컬 테스트 범위에 포함되지 않는다.


### 2026-09-18 제안 편집과 직접 경기시작
- 운영진의 편집 기기에서는 점선 제안 그룹의 회원을 드래그로 추가·제거·교체한다. 수정 명단도 작성자와 운영진에게만 동기화된다.
- 제안 하단은 `X + 자동매칭`, 4명일 때 `X + 경기시작`. X는 일반 그룹과 같은 크기이며 거절한다. 작성자는 제안 취소만 가능하다.
- 일반 그룹도 별도 매칭확정 단계 없이 4명에서 경기시작을 누르면 바로 시작한다. 경기중 편집 버튼은 경기완료 왼쪽에 둔다.
- 제안 편집은 `updated_at` 비교로 동시 수정 손실을 막고, 제안 시작은 편집 권한·대기 상태·중복 편성·코트 점유 검증 후 실제 경기 생성과 제안 종료를 같은 트랜잭션으로 처리한다. 응답 유실 뒤 재요청은 기존 경기 결과를 반환한다.

### 제안 중복 자석과 회원 찾기
- 작성자·운영진 화면에서는 보낸 제안의 회원을 자유 자석 풀에서 숨긴다. 비공개 제안이 보이지 않는 회원 화면의 자유 자석에는 영향을 주지 않는다. 실제 팀 소속은 변경하지 않으며 제안에서 빠지거나 취소·거절되면 자유 자석으로 다시 보인다.
- 빈 보드의 같은 위치를 두 번 탭하거나 돋보기 버튼을 누르면 회원 찾기를 연다. 이름·초성 검색 결과는 현재 화면의 자유 자석·일반 그룹·경기중 코트·비공개 제안에서 모으며 같은 회원은 한 번만 표시한다.
- 결과 선택 또는 Enter로 검색창을 닫고 해당 회원의 자석에 4초간 노란 테두리·화살표와 빛을 표시한다. 동작 줄이기 설정에서는 깜박임 없이 표시한다. 검색·강조는 기기 내에서만 처리하며 편집 권한을 요구하거나 서버에 저장하지 않는다.
- 빈 공간의 드래그·오래 누르기·멀리 떨어진 두 탭·핀치는 검색을 열지 않는다. 기존 자석의 더블탭 및 드래그 동작을 유지한다.
