...
| | 다른 자유자석 ≤`PAIR_RADIUS` | `createPair` (원본 팀에서 빠져 새 페어로 이동) |
...
- **박스 하단 CTA 버튼**(`TeamBackground`): 라벨은 **팀 상태/액션 기준(편집 권한 무관)** 으로 정한다 — 구성중(2명+)=**"우선배치"** 토글 / 4명 시작 가능=**"매칭확정"**(초록) / 확정됨=**"경기시작"**(주황, 코트 없으면 "코트 대기") + 좌측 ✕(확정취소). 코트가 비면 다음 경기 팀의 경기시작 버튼이 반짝임(`CTA_PLAY_FLASH`). **비활성=회색**.
