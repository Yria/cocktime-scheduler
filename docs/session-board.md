...
- 묶음 아래 기존 매칭확정과 같은 형태의 **매칭 제안** 버튼을 누를 때만 서버에 저장한다. 전송 중에는 명단 편집과 중복 전송을 막고, 실패하면 묶음을 유지한다. 같은 요청 ID를 재시도해도 중복 제안이 생기지 않는다.
- 보낸 제안은 **작성자와 운영진 보드에 점선 테두리 그룹**으로 표시한다. 작성자 화면에서는 전송 전 위치를 유지한다. 별도 목록·상단 제안 버튼은 없다. 작성자는 그룹 아래에서 취소할 수 있고, 운영진은 확인하거나 **X로 거절**할 수 있다. 취소·거절은 양쪽 화면에서 사라지며 종료된 제안을 늦은 확인 요청으로 되살릴 수 없다. 제안은 실제 팀을 자동으로 만들거나 출전 순서를 예약하지 않는다.
...
- 운영진 경기 여부와 무관하게 제안할 수 있다. 운영진의 기존 편집권, 팀 확인·시작·완료 흐름은 유지한다.
...
검증: `node supabase/tests/match_proposals.test.mjs`는 로컬 PGlite에서 실제 SQL과 역할별 RLS, 1~4명 검증, 재시도, 취소·확인 권한, 공유 보드 불변을 확인한다. `e2e/match-proposals.spec.ts`는 외부 요청을 가로채 실제 보드 드래그·전송·실패 보존·역할별 화면과 모바일/데스크톱 캡처를 확인한다. 실제 운영 Supabase의 Realtime 전달은 이 로컬 테스트 범위에 포함되지 않는다.
