...
- 조회 경계는 화면 필터가 아니라 `match_proposals`의 SELECT RLS다. 작성자와 운영진만 읽으며, 명단에 포함된 다른 회원도 볼 수 없다. INSERT/UPDATE는 검증된 RPC만 허용한다. 작성자는 현재 세션 참가자여야 하고 명단은 같은 세션의 중복 없는 1~4명이어야 한다.
