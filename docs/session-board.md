...
일반 회원에게는 운영진용 보기 전용 프레임·권한 버튼을 표시하지 않는다. 세션에 참여한 회원은 보드에서 원하는 회원을 끌어 1~4명 묶음을 만들 수 있다. 빈 곳에 놓으면 1명 묶음, 다른 자유 자석 위에 놓으면 2명 묶음, 묶음 위에 놓으면 최대 4명까지 추가한다. 묶음 밖으로 빼거나 더블탭하면 선택이 해제되고, X는 묶음 전체를 해제한다. 경기 중이거나 실제 팀에 속한 회원도 제안에 담을 수 있지만 원래 경기·팀 소속은 바뀌지 않는다.

- 전송 전 묶음은 `matchProposalStore`의 메모리에만 존재한다. 공유 `boardStore.drafts`·예약·경기 상태와 별개이며 새로고침, 보드 이탈, 계정·세션 변경 시 사라진다.
- 묶음 아래 **매칭 제안**을 누를 때만 서버에 저장한다. 전송 중에는 명단 편집과 중복 전송을 막고, 실패하면 묶음을 유지한다. 같은 요청 ID를 재시도해도 중복 제안이 생기지 않는다.
- 보낸 제안은 작성자의 **내 매칭 제안**, 운영진 보드 상단의 **매칭 제안**에서 확인한다. 작성자는 취소할 수 있고 운영진은 확인 상태를 표시할 수 있다. 제안은 실제 팀을 자동으로 만들거나 출전 순서를 예약하지 않는다.
...
배포 시 `supabase/migrations/20260918010000_private_match_proposals.sql`을 프런트 배포 전에 적용해야 한다. 새 테이블·RLS·RPC와 Realtime publication 등록을 포함한다. 취소는 행 삭제가 아닌 상태 변경으로 전달한다. 이는 [Supabase Postgres Changes 문서](https://supabase.com/docs/guides/realtime/postgres-changes)의 DELETE 이벤트 RLS 제약을 피하기 위한 구성이다.
