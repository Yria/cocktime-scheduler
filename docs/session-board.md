...
> **2026-09-13 임시 비활성화.** `SessionBoard.tsx`의 `MEMBER_PARTY_ENABLED = false`로 버튼·안내뿐 아니라 `useMemberParty`의 조회·참여·자동 확정도 실행하지 않는다. 재개 시 `true`로 되돌리고 프론트를 배포한다. 아래 설계와 서버 RPC는 유지한다.
