...
> 이 서열은 `W_SKILL(1.5·k²) < W_GROUP2(8) < W_GAME(10) < W_GROUP3(24) < W_PLAYING(30) < W_GENDER(50) < W_GROUP4(60)` 로 코드(`RECOMMEND_WEIGHTS`·`DEFAULT_WEIGHTS`)에 반영돼 있다(7절·2절). 실력 항은 확장폭 k의 **제곱**이라 서열상 위치가 k에 따라 움직인다 — k≤2는 경기수 1판 아래(타이브레이크), k=3부터 경기수를 넘고, k=6(54)이어도 완전 재결성(60)보다는 싸다("재결성될 바엔 벌어진 팀"). 과거엔 `W_SKILL`이 최우선(20)이라 실력이 비슷한 사람끼리 반복해서 같은 경기에 뽑히는 부작용이 있었다.
...
`W_SKILL, W_SKILL_EXP, W_GROUP2, W_GROUP3, W_GROUP4, W_GAME, W_MIXED, W_WAIT` 8개.

기본값(`DEFAULT_WEIGHTS`): `W_GAME 10.0, W_GROUP2 8.0, W_GROUP3 24.0, W_GROUP4 60.0, W_SKILL 1.5, W_SKILL_EXP 2, W_MIXED 0, W_WAIT 0`. (재결성 회피는 겹침 수에 따라 경기수 1판 아래(`W_GROUP2`)부터 위(`W_GROUP3·4`)까지 걸친다 — 위 핵심 철학 참조. `DEFAULT_WEIGHTS`는 `rankCandidates`를 weights 없이 호출할 때의 폴백이며, 실제 추천/자동편성은 `RECOMMEND_WEIGHTS`(7절)를 쓴다.)
...
| `W_SKILL` | 1.5 | **실력은 3순위** — 스프레드 증가분(밴드 확장)에만 벌점. 2v2 균형은 페어 편성(`pairPlayers`)이 보정 |
...
4. **오래 쉰 사람 우선 (`W_WAIT`, 2절 base에서 반영)**: 대기 경과 분(分)에 비례해 점수를 낮춘다(`−waitMinutes·W_WAIT`). 8명 1코트처럼 4/4가 딱 떨어질 때 "누군 2번 연속·1번 휴식 / 누군 2번 휴식·1번 출전"으로 갈리는 연속 휴식 편차를 완화 — 같은(또는 비슷한) 판수면 **더 오래 기다린 사람**이 다음 경기에 확실히 들어온다. `waitSince`는 경기 완료/휴식 복귀/콕확인에서 갱신된다(자연 리셋). `W_WAIT`(분당 가중)는 실제 휴식 시간에 비례하는 **튜닝 knob** — 너무 세거나 약하면 조절.

   > **경기중 후보는 대기 항이 0이다(2026-08).** `assign_match`가 `wait_since`를 리셋하지 않아, 이 가드가 없으면 코트에 서 있는 시간이 "기다린 시간"으로 계산된다. 실측 격차 +9.3점이 `W_PLAYING`(30)을 상쇄하고, 진행 중인 판이 아직 `game_count`에 안 잡히는 −10과 합쳐 **실효 ghost 페널티가 약 11(≈`W_GROUP2`)까지** 내려가 있었다. 게다가 경기가 길수록 더 싸지는(분당 1점) 변동값이었다. 가드로 실효값이 **20에 고정**된다.
   >
   > **알려진 한계 — 이 항에는 상한이 없다.** 프로덕션 실측(편성 시점 대기 후보 4,849건)으로 대기 보너스가 단독으로 `W_GAME`(10)을 넘는 후보가 **16.5%**, `W_GROUP3`(24) 초과 **1.4%**, `W_PLAYING`(30) 초과 0.9%, `W_GROUP4`(60) 초과 3건(max 63분). 즉 드물게 "오래 기다렸다"가 "같은 4명 재결성 금지"를 이긴다. 눈금을 경기 순번(`match_assign_count` 기반)으로 바꾸면 이산이라 구조적 상한이 생긴다 — 후속 과제.
   >
...
- **공칭 30 vs 실효 페널티**: 진행 중인 경기는 아직 `game_count`에 반영되지 않으므로(완료 시 +1) 경기중 후보는 판수 1판(`W_GAME` 10)만큼 유리하다. 따라서 실효 페널티는 **20**이다(대기 항 가드 적용 후, 3절·7절). 가드 이전에는 여기에 대기 보너스까지 상쇄돼 약 11까지 내려가 있었다.
