...
> DB 측 `complete_match` RPC 는 실제 **팀 페어 2쌍**(teamA p1-p2, teamB p1-p2)만 누적한다(차이 주의).
...
|---------|------|------|
| 0 | `남복` | |
| 1 | `혼합` 또는 `남복` | 해당 여성이 `allowMixedSingle` 이거나 `singleWomanIds`(혼합 허용 목록)에 포함되면 `혼합`, 아니면 `남복` |
| 2 | `혼복` | |
| 3 | `남복` | 남자 1명 포함 |
| 4 | `여복` | |
