...
> **2026-09-24 현재 동작:** 네 명 선발의 우선순위는 [네 명 편성 정책](TEAM_SELECTION_POLICY.md)이 기준이다. 아래 과거 합산 점수·greedy 설명은 현재 `recommendTeammates`의 조합 탐색과 다르다. 이번 변경은 선발 가중치를 바꾸지 않고 **예비팀의 합류 대기 자리**와 경기 완료 후 채움을 추가한다.

## 합류 대기 자리 (2026-09-24)

- 진행 중인 경기가 있고 다음 중 하나면 새 예비팀의 자동편성은 기본 2명만 고른다(`prefersWaitingDraft`). 수동으로 고른 3명은 유지한다.
  - 바로 출전할 준비가 된 팀 수가 `max(1, 빈 코트 수)` 이상이다.
  - **대기자만으로 허용 네 명을 만들 수 없다(2026-09-25).** 특정 경기 중 선수를 예약하는 대신 이름 없는 자리로 기다린다. 확정 멤버와 자유 대기자를 합쳐 2명 이상이고 그중 대기자가 있어야 한다. 한 명뿐이면 기존처럼 예약해 네 명을 만든다.
  - 기본값이 대기인데 대기팀을 만들 수 없으면(대기자 1명뿐 등) 예약 팀으로 대신 만들지 않고 안내한다. 추천 창은 닫히지 않으므로 대기 모드를 끄고 다시 누르면 예약으로 만든다. 추천 창에서 체크박스를 직접 켜거나 끈 경우는 그 선택을 따른다. 예약 팀으로 대신 만드는 변형과 비교하면 기본 순서에서는 결과가 같았다. 차이는 새 팀을 코트보다 먼저 만드는 순서에서만 났고 서로 엇갈렸다. 세션 324 3팀에서 대신 만들면 같은 4명 연속 경기가 0.5%→1.6%, 최장 대기가 14.7→16.2분으로 나빴다. 빠듯한 세션 175·192 3팀에서는 이 변형이 코트 빈 시간을 3~5분 덜 남겼다.
  - 근거(대기열 재생, 여유 인원 1~4명인 세션 175·209·156·192, 50시드, 기본 순서와 새 팀 먼저 순서 × 1~3팀 = 24개 조건): 예약 대신 대기 자리를 쓰면 HEAD 대비 코트 빈 시간이 모든 조건에서 5~40분 줄고(시드 35~50/50에서 개선), 3명 재회 경기 비율도 모든 조건에서 준다(예: 세션 156 2팀 58.8%→14.9%, 세션 209 2팀 47.2%→18.6%). 대가도 있다. 판수 표준편차는 세션 192 전 조건과 세션 175 2~3팀에서 0.10~0.18 늘었다(시드 30~43/50에서 악화). 최장 대기는 세션 175 2~3팀에서 1.5~5.4분, 세션 209·156의 새 팀 먼저 3팀에서 2.0~2.5분 늘었다. 같은 4명 연속은 세션 175 1팀에서 14.3%→18.1%, 세션 192에서 6개 조건 중 5개에서 0.4~4.0%p 늘었다.
- 1~3명이 있는 코트 그룹과 **이미 있는 일반 예비팀의 카드 `자동매칭`**은 지금 네 명을 채운다(필요하면 경기 중 예약). 이미 있는 팀의 추천 창 기본값도 팀 상태를 따르므로, 대기 기본값은 새 팀과 시드에만 적용한다. 빈 코트 그룹(0명)은 아래 '코트 공백 방지' 순서로만 채우고, 아무것으로도 못 채우면 안내한다.
- `DraftTeam.waitForCompletion`은 이름 없는 나머지 슬롯을 뜻한다. 일반 빈칸이나 특정 경기 중 선수의 `Reservation`과 구분하며, 보드 저장·복원과 원격 비교에도 포함한다. 4명이 되면 해제한다. 기존 인원 하한 2명은 유지한다.
- 용어(2026-09-25): 보드의 '합류 대기'는 이 이름 없는 빈자리만 뜻한다. 4명을 골랐지만 예약한 선수가 아직 경기 중인 그룹은 코트 그룹이든 예비팀이든 '예약 대기'로 표시한다(예전 코트 라벨 '합류 대기'를 바꿈).
- `autoFillTeammates`의 `targetSize: 2`는 대기자만 대상으로 부분 조합을 비교한다. 4명 선발의 기본 동작과 가중치는 그대로다. `acceptCompletion`은 아래 여러 팀의 완성 가능성을 보존하기 위한 제약이다.
- **채우는 시점은 경기 완료뿐이다(2026-09-25).** 경기 완료가 성공하고 같은 세션이면 기존 선수 예약을 먼저 해소한다(편집 권한과 무관, 기존 동작). 이어 편집 권한이 있으면 `planWaitingTeamFills`로 표시된 예비팀을 채운다. 이때 빈 코트에 1~3명만 올라간 코트 그룹이 있으면 그 그룹과 대기팀을 한 번에 배분한다. 코트 그룹이 완성되는 수를 줄이지 않는 범위에서 대기팀을 최대한 완성한다. 코트 그룹 몫은 채우지 않고 자유 상태로 남겨 코트 버튼으로 채운다. 빈 코트 매칭 버튼과 대기팀 카드 버튼은 대기팀을 채우지 않는다. 새 복귀자뿐 아니라 기존 자유 대기자도 후보이며, 타 팀 멤버·예약자·경기 중·휴식·미확인·경기 명단 수정 중 선수는 제외한다.
- 성별 구성의 가능한 배분을 미리 계산해 **완성 가능한 팀 수를 최대화**한다. 같은 수를 완성할 수 있으면 오래된 팀을 먼저 채운다. 각 팀의 후보는 기존 출전·중복·실력 우선순위로 선택한다. 전체 팀의 실력/중복 점수까지 전역 최적화하는 것은 아니다. 한 선수는 한 팀에만 들어가며 네 명을 완성할 수 없는 팀은 그대로 둔다.
- **코트 공백 방지:** 빈 코트에서 매칭을 누르면 완성된 다음 팀을 먼저 옮긴다. 자유 대기자만으로 부족하면, 특정 선수 예약이 없는 미완성 합류 대기팀의 인원까지 후보에 넣어 재조합한다. 두 대기팀에 2명씩 묶인 네 명이 출전 가능한데도 후보에서 빠지는 상황을 방지한다. 일반 수동 팀·완성 팀·특정 선수 예약은 재조합하지 않는다. 이미 1~3명이 있는 코트 그룹을 이렇게도 못 채우면 기존처럼 `autoFillTarget`으로 경기 중 선수를 예약해 완성한다(예약이 섞인 그룹 포함). 빈 그룹(0명)은 예약하지 않는다.
- **대기팀 카드의 버튼은 `완료 후 채움` 상태 표시(비활성)다.** 예전 `지금 채우기`(즉시 채움)는 제거했다. 코트 버튼 안에서 먼저 채우던 동작과 함께, 방금 준비한 대기팀을 같은 벤치 인원으로 바로 완성해 기존 방식과 같은 재회를 만들었다(세션 324 재생 100시드: 3명 재회 경기 비율이 2팀 즉시 채움 79.5%, 3팀 새 팀 먼저 편성 77.4% → 수정 후 0.6%·5.3%, 기본 순서 0.6%·5.5%와 비슷함. `tools/team-simulator/README.md`). 지금 채우려면 빈자리를 눌러 추천 창에서 선수를 고르거나, 대기 모드를 끄고 자동편성을 누른다. 이미 있는 합류 대기팀에 새로 고른 사람이 없으면 추천 창의 자동편성은 비활성이다. 새 팀을 대기로 만들 수 없으면 창을 닫지 않고 안내한다. 스토어의 `autoFillTeam`도 대기팀에는 안내만 하고 채우지 않는다.
- 모든 자동 채움(코트 버튼, 코트 그룹의 예약 대체 경로, 추천 창·카드의 자동편성, 완료 후 채움)은 다른 코트의 '선수 변경' 명단에 올라간 선수를 쓰지 않는다. 그 선수는 보드와 추천 창 후보 목록에서 숨겨지고, 빈 코트는 그런 선수가 든 준비된 팀 대신 다음 준비된 팀을 옮긴다. 대기 기본값 판단도 그 선수를 대기자로 세지 않는다.
- 코트 배정과 경기 시작은 기존 버튼으로 한다. 경기 완료는 예비팀만 채우며 자동으로 경기를 시작하지 않는다. 인원·성별 조건이나 기존 수동 예약 때문에 허용 조합이 없는 경우까지 코트 가동을 보장하지 않는다.
...
> | `planWaitingTeamFills` | `src/lib/teamSelection/fillWaitingTeams.ts` | 경기 완료 후 합류 대기팀 채움 계획(완성 팀 수 최대화 → 오래된 팀 우선) |
> | `prefersWaitingDraft` | `src/lib/board/waitingDrafts.ts` | 새 예비팀 자동편성의 합류 대기 기본값 판정(준비된 팀 수 또는 예약 필요 여부) |
...
**추천 팀원 모달 하단의 자동편성 버튼**이 빈 슬롯을 한 명씩 채운다. 새 팀·자유 선수 시드·기존 팀에서 사용할 수 있다. 구성 중 팀 박스의 CTA `자동매칭`(`autoFillTeam`)도 같은 경로를 쓰되, 대기 모드 없이 지금 네 명을 채운다. 합류 대기팀의 CTA는 `완료 후 채움`, 4명이지만 예약자가 경기 중이면 `예약 대기` 비활성 안내다.
...
```
autoFillTeammates(confirmed, pool, ctx, count, weights?, { maxPlaying, targetSize, acceptCompletion }):
...
- `targetSize`(2~4, 기본 4): 이 인원까지만 고른다. 합류 대기팀 생성은 `targetSize: 2`로 대기자 두 명 조합만 비교한다. 범위 밖 값이면 빈 배열.
- `acceptCompletion(picks)`: false인 조합은 후보에서 뺀다. `planWaitingTeamFills`가 다른 대기팀의 완성 가능성(성별 구성)을 보존하는 데 쓴다.
