...
- 직전 혼복 출전자(규칙 1.5) 후순위 처리 후, 남은 후보 중 실력 차이가 가장 작은 쌍 선택.
- 예시: 여자 A·B(스킬 전부 X)와 C(스킬 일부 X)가 있으면 A+B를 우선 선발하고, C는 다음 게임에서 규칙 0(경기 횟수 균등)에 의해 자동으로 선발 우선순위가 높아진다.
...
1. 여자 ≥ 2명 + 남자 ≥ 2명 → 혼복 (규칙 1·2 적용해 남자 선발, 규칙 2.5 적용해 여자 선발)
...
**경기 횟수 균등(규칙 0)과 혼복 횟수 균등(규칙 1)은 독립적으로 작동해야 한다.**

- 어떤 선수가 이번 경기에 출전하는지(규칙 0)와 혼복이 편성될 때 남자를 누구로 선발하는지(규칙 1·2)는 서로 간섭해서는 안 된다.
- 혼복 여부 판단과 여자 2명 선발은 규칙 0에 의해 우선순위가 결정된 선발 대상 안에서 이루어진다.
- 혼복 남자 2명 선발은 **대기 중인 남자 전원**을 대상으로 규칙 1·1.5·2를 적용한다.

### 단계별 선발 대상 범위

| 선발 단계 | 대상 범위 |
|-----------|-----------|
| 혼복 편성 여부 판단 | 규칙 0 기준 상위 선발 대상 |
| 여자 2명 선발 | 규칙 0 기준 상위 선발 대상 중 여자 |
