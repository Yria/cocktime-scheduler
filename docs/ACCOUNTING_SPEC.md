...
- **수동 부과 카드**(§3.6): 그 달 배치(`charged_on`)를 회비·대관비 카드와 **같은 언어**로 세운다 — 제목+종류 배지 → 판정 두 줄(`지출 연결` · `수납 N/M · 미납 K`) → 진행 막대 → 금액 사실(인당·부과합·받은 돈·클럽 부담). **마감 판정도 세션과 같다: 지출 연결 + 미납 0.** 지출 = 그 묶음(`dues_batches.id`)에 붙은 출금 합계 — 안 붙으면 공개 회계에서 미분류로 남으므로 세션의 `코트지출 연결`과 같은 무게로 다룬다(그 달 거래만 보므로 결제가 다음 달로 넘어가면 '미연결'로 보인다 — 세션 카드도 같은 성질). 지출이 부과합보다 크면 차액을 `클럽 부담`으로 적는다(회식 지원처럼 의도된 경우가 많아 경고가 아니다). [명단·수정 ›] 는 §3.6 의 **그 배치를 열고** 들어간다(`?open={batch_key}`). 계산 = `manualCards.buildManualCards`, 카드 = `ManualBatchCard` — [부과] 탭 목록과 **같은 컴포넌트**다(같은 부과인데 탭마다 다르게 보이던 것을 없앴다, 2026-08-23). 화면 순서는 부과 카드끼리 모은다: 회비 → 이월 → 수동 부과 → 예정 세션 → 세션.
- **카드 공용 시각 언어**: 배지(`pill`)·판정 아이콘(`mark`)·진행 막대(`Meter`)·상세 링크(`MoreLink`)·겉면(`CARD_CLASS`/`cardBox`)은 `duesCardStyles`(스타일)·`duesCardBits`(컴포넌트) 두 파일이 단일 소스다(react-refresh 규칙 때문에 갈라 둠). **✓/! 는 "사람이 처리할 일이 남았는지"만 말한다** — 금액·인원 같은 사실은 마크 없이 적는다(빨간 !가 흔해지면 아무도 안 본다).
...
- 목록 = 그 달 배치(`charged_on` 기준), 카드는 §3.1 과 **같은 `ManualBatchCard`**. [명단·수정]으로 명단·금액을 고쳐 저장(같은 `batch_key` upsert)하거나 삭제.
...
- **열린 시트는 URL 이 진실**이다(`?open=new` | `?open={batch_key}`). 그래서 §3.1 카드가 그 배치를 바로 열 수 있고, 효과 안에서 `setState` 로 시트를 여는 경로가 없다(cascading render 금지 규칙과도 맞다). 목록에 없는 키(다른 달·지워진 배치)면 시트를 열지 않는다.
