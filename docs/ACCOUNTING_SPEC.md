...
### 1.1 금액
- **회비(monthly_fee)**: 5,000원/월. 회원(활성·비운영진·비게스트·비명예회원) 대상, 월 단위(`period_ym`). 비활성(정지)이 되면 새 부과가 안 생기고 **그 회원의 미납분도 면제된다**(납부분·대관비는 보존 — §4). **명예회원**(`members.is_honorary`)은 회비 면제(§4).
...
- **회비 미납 행 액션 = [이월] · [면제]**, 그리고 그 달 면제분은 **면제 N명** 접힘에서 [되돌리기](2026-08-19). [면제]는 `dues_set_charge_status(id,'waived')`, [되돌리기]는 `'reset'`(둘 다 `dues_audit_log`). 부분납(배분 있는) 행에는 [면제]가 붙지 않는다 — waived 로 둔 뒤 대사취소하면 `dues_alloc_sync` 가 `amount_paid` 만 0으로 되돌리고 status 는 waived 를 유지해 잔액이 모든 미납 뷰에서 사라진다. **이 목록이 정지 시 자동 면제(§4)를 감시하는 창**이다 — 자동으로 처리된 건이 여기 보이고 여기서 되돌려진다. 종전에는 진입점이 없어 운영진이 SQL 로 손질했다.
...
- **회비 룰**: `is_active AND not is_guest AND not is_honorary AND not 운영진`, 가입월(`membership_started_at ?? created_at` + `offset_days`) 다음 달부터 `amount_due=회비액`.
  - **비활성 회원은 부과 대상이 아니고, 그 규칙은 이미 생성된 미납에도 적용된다** (2026-08-20, `20260820000000` + `20260820010000`).
    - **생성**: 활성 회원만. 나간 사람에게 매달 새 회비가 붙으면 영구 미납이 쌓인다.
    - **잔재**: `members.is_active` 가 true→false 로 바뀌면 트리거 `trg_members_waive_dues_on_deactivate` 가 그 회원의 **미납(`status='unpaid'` AND `amount_paid=0` AND `deferred_to is null`) 회비를 `waived`** 로 돌린다. 생성만 막고 잔재를 남기면 규칙이 반만 적용돼 미납 현황에 영구히 쌓인다(운영진이 매번 손으로 지우던 것). 선례는 회비 안에 있다 — 명예회원 지정(`dues_set_honorary`)도 자격이 사라지는 순간 이미 생성된 미납을 정리한다.
    - **건드리지 않는 것**: 납부·부분납(`amount_paid>0` — 받은 돈은 받은 돈), **대관비**(실제 코트 사용 대가라 탈퇴와 무관), 이월 중인 건(`deferred_to` 있음 — waived 로 돌리면 이월 목록에서 '정산 완료'처럼 보인다).
    - 삭제가 아니라 `waived` 라 행이 남아 "왜 안 걷었나"가 감사(`dues_audit_log.waive_dues_on_deactivate`, charge id·period_ym 포함)로 추적되고, **회비 현황 [면제 N명] → [되돌리기]** 로 사람이 되돌릴 수 있다. **재활성화는 면제를 자동 복구하지 않는다.**
    ...
    - **판단이 두 번 뒤집힌 절이다.** 8/18 도입 → 8/19 폐지(근거: 홍예린 님이 7/20 실제 참석) → 8/20 복원. 폐지 근거가 틀렸다 — **회비는 참석 대가가 아니라 월 회원비**라 참석 여부는 부과 근거가 아니다(참석 대가는 대관비). 7월 정산이 풀린 진짜 원인은 하드삭제 CASCADE 였고 그건 아래에서 따로 봉인했다. 복원 시 8/18 버전의 실제 결함 두 개를 고쳤다: `deferred_to` 가드 추가, 감사 로그에 charge id·period_ym 기록(8/18 버전은 member·건수만 남겨 정확히 되돌릴 수 없었다). 그리고 **자동 면제를 다시 켤 수 있게 된 전제**는 8/19 에 넣은 [면제 N명] 목록·[되돌리기] 다 — 그전에는 자동 면제분이 화면 어디에도 안 남아 묻혔다.
