...
- **정원 초과 확정자(프리패스)가 회식비·수동부과 엔빵 분모에 들어간다.** 대관비는 프리패스가 **부과 없는 장소에서만** 열리므로 영향이 0원이지만(`charges_court_fee=false` → `dues_generate_session_court` 가 0 반환), 무부과 장소의 정모도 수동부과 대상이라(§1.1) 회식비 인당이 최대 4명분(운영진 2 + 신규 2)만큼 내려갈 수 있다. 이미 낸 사람은 옛 금액이 유지되므로(`amount_paid > 0` 은 자동정리 제외) 한 묶음에 두 금액이 남는다 — 잔돈은 §5 돌려받을 돈으로 보인다.
