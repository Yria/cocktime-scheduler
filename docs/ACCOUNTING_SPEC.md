...
- **회비 룰**: `is_active AND not is_guest AND not is_honorary AND not 운영진`, 가입월(`membership_started_at ?? created_at` + `offset_days`) 다음 달부터 `amount_due=회비액`. **단 합류월은 하한이 걸린다**(바로 아래).
  ...
  - **탈퇴는 소프트(하드삭제 전면 금지)** (2026-08-19, `20260819010000_member_soft_delete_only.sql`): `delete_my_account` 가 회원 행을 지우지 않고 `is_active=false` + `auth_user_id/개인정보/푸시구독` 정리로 바뀌었고, RLS `members_delete` 정책과 authenticated 의 DELETE 권한도 회수했다.
    - **그때 탈퇴가 조용히 죽었다** (2026-08-19 ~ , **의도적으로 안 고침** — 아래 판단 참고): 같은 마이그레이션이 감사 로그를 `ops_audit` 에 **직접 INSERT** 하면서 `kind='member_self_soft_delete'` 를 썼는데 `ops_audit.kind` 의 CHECK 는 `('attendance','counter','session')` 그대로였다 → 23514 로 **트랜잭션 전체 롤백**, 회원은 "탈퇴 처리에 실패했어요" 만 봤다. 전부-또는-전무라 데이터 손상은 없었고 기능만 죽었다(지문: `ops_audit` 에 member 계열 행 0건).
    - **왜 두 달 가까이 안 보였나**: 다른 감사 호출부는 전부 `ops_audit_write()` 를 거치고 그 함수는 `exception when others then null` 로 **모든 실패를 삼킨다**. 잘못된 kind 를 넘겨도 조용히 버려질 뿐 호출자는 성공한다. 그 래퍼를 안 쓴 유일한 곳이 `delete_my_account` 였고, 그래서 유일하게 죽었다.
    - **안 고치기로 한 판단** (2026-08-21): 실제 운영은 **회원이 스스로 나가지 않고 운영진이 [비활성] 로 처리**한다 — 본인 탈퇴 경로를 쓰지 않으므로 수선 실익이 없다. **대신 알고 있어야 할 두 가지**: ① 회원 화면 [회원 탈퇴] 버튼(`ProfileSetup.tsx`)은 아직 살아 있어 누르면 실패 문구를 본다 — 없애려면 버튼을 숨겨야 한다. ② 아래 §13 의 '본인 탈퇴로 그 달 회비 회피' 잔여 위험은 **지금은 성립하지 않는다**(탈퇴 자체가 실패하므로). 나중에 탈퇴를 되살리면 그 구멍이 함께 열린다.
    - **되살릴 때의 처방**(검증 완료, 프로덕션 롤백 드라이런): ① `ops_audit_kind_check` 에 **도메인 값 `'member'`** 추가하고 행위는 `detail.action='self_soft_delete'` 로 내린다(kind 에 행위를 적으면 이벤트마다 CHECK 를 고쳐야 한다) ② 그 INSERT 를 `begin … exception when others then null; end;` 로 감싸 **감사 로그 실패가 탈퇴를 죽이지 못하게** 한다 — 근본 원인은 kind 오타가 아니라 그 구조다. `ops_audit_write` 를 그대로 쓰면 안 된다: 그 함수는 actor 를 `current_member_id()` 로 채우는데 탈퇴는 그 직전에 `auth_user_id` 를 끊어 **actor 가 NULL 로 떨어진다**(실측). actor=본인을 남기려면 직접 INSERT 가 맞다.
...
- **본인 탈퇴가 그 달 미납 회비를 지운다** — 잔여 위험이지만 **현재는 성립하지 않는다**: `delete_my_account` 자체가 `ops_audit` CHECK 누락으로 실패하기 때문이다(§4 탈퇴 항목, 안 고치기로 판단). 탈퇴를 되살리는 순간 이 구멍도 함께 열린다 — 월말 탈퇴로 5,000원을 피하고 새 계정으로 재가입하는 경로. 계정·로그인·이력을 다 잃고 운영진 눈에 중복 행으로 보이는 값비싼 회피라 그때도 룰로 막을 필요는 없어 보이지만, **모르고 열리는 것과 알고 두는 것은 다르다**.
- **회원 화면 [회원 탈퇴] 버튼이 실패한다** (`ProfileSetup.tsx`) — 위와 같은 원인. 운영은 운영진 [비활성] 으로 하므로 기능은 안 고치기로 했고, 그러면 **버튼을 숨기는 것**이 남은 정리 항목이다.
