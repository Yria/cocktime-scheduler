...
2. `부과 대상 − 누락 − 발행대기 − 부과삭제 + 대상아닌부과 = 실제 부과 건수`

**구성**: ①인원 대조(위 항등식) ②금액 대조(낼 돈 → 받은 돈·미납·부과삭제 / 비회원 입금 / 코트 지출 → 현재 순액·전원완납시 순액. **발생 기준** — 회계 §3.3의 월 통장 현금주의와 다르다) ③**전체 명단**(접힘 토글).

**전체 명단 = 한 목록**(`SessionSettle.roster`). 부과 있는 사람 + 부과 누락 + 발행 대기 + 정상 면제를 섹션으로 쪼개지 않고 한 목록에 세우고 **우측에 사유**를 적는다 — 쪼개면 "이 사람 어디 있지"를 여러 곳에서 찾아야 해서 대조가 안 된다. 행 구성: `이름 · 딱지(당일취소/게스트 대납/운영진/삭제함) · 사유 · 금액 · 상태칩 · [부과삭제|되돌리기]`. 부과 없는 행은 금액·상태칩 자리가 비고 사유만 놓인다.
- **정렬 = 이름 가나다순**(`localeCompare(…, "ko")`). 사람을 이름으로 찾는 게 이 명단의 용도다 — 확인 대상은 우측 사유가 빨강으로 떠서 눈에 걸린다. `RosterKind` 순위(`missing→stale→pending→charged→orphan→exempt`)는 동명이인 tie-break 로만 쓴다.
- **한 사람은 한 줄**(테스트로 강제). 네 소스가 겹치지 않게: `missing/pending ⊂ targets − 부과보유자`(서로도 배타), `exempt ⊂ nonTarget − 부과보유자`, 부과는 `(member, session)` 유니크. **`exempt` 에서 부과 보유자를 빼는 게 핵심** — 안 빼면 grace 인데 `void` 부과가 남은 사람이 '면제'와 '무효 부과' 두 줄로 보인다.
- **판정 순서: 자리를 잡았는지 → 운영진.** 운영진 판정을 앞세우면 사전취소·대기한 운영진까지 `운영진 · 대관비 면제`로 명단에 올라와 오지도 않은 사람이 면제로 보인다(실측 #106에서 운영진 면제가 1명인데 3명으로 뜸).
- **사유 문구는 부과 유무로 갈린다**: 같은 `NonTargetReason` 이라도 부과가 있으면 `EXTRA_LABEL`("사전취소인데 부과됨", 빨강), 없으면 `EXEMPT_LABEL`("운영진 · 대관비 면제", 회색). 부과가 아직 없으면 뭉뚱그린 `참석·당일취소` 대신 대상 근거를 그대로 쓴다: `참석 확정 / 당일취소 / 보드 참가` + `부과 미생성 / 발행 대기 / 종료 후 자동 부과`.
- **발행 대기와 실제 누락은 다르다**: 진행 중 회차의 미발행분과 `dues_charge_drafts` 에 있는 사람은 `pending` 으로 세워 `⚠확인`에서 뺀다. closed 회차에서 발행 행도 초안도 없는 대상만 `missing`이다. 실측 8/30 #165의 `확인 12`는 대상 28명 중 발행 16명·발행대기 미발행 12명을 누락으로 뭉친 오표시였다.
- **헤더 카운트 = 배타적 6칸**: `완납 / 미납 / 무효 / 부과없음 / 발행대기 / ⚠확인`. **확인필요 행은 납부 상태로 이중 계상하지 않는다** — 잔재(완납이면서 확인필요)가 `완납`에도 잡히면 헤더가 분할처럼 읽히면서 합이 안 맞아 보인다. 확인이 필요하다는 게 그 사람에 대한 더 중요한 사실이므로 그쪽으로 센다. 불변식 `paid + unpaid + dead + none + pending + flaggedCount = roster.length`(테스트로 강제). `flaggedCount` 는 카드 ⚠배지와 **같은 단일 소스**(양쪽이 따로 더하지 않는다). 0은 생략.
...
- **남는 돈은 오류가 아니라 환불 대상이다**(2026-08-24): 부과 선택액 < 정산 대상액이면 경고를 "금액이 안 맞아요"로 뭉개지 않고 `N원이 남아요 — 확정하면 그 회원 화면에 "돌려받을 돈 · 환불 계좌번호를 알려주세요" 안내가 떠요`로 말한다(확인 다이얼로그도 같은 문구, `tone` 은 danger 아님). 클럽 수입으로 둘 거면 묶음을 고르라고 함께 안내한다 — 묶음이 붙으면 §3.4 안내는 뜨지 않는다.
...
- **돌려받을 돈**(2026-08-24): 낼 돈보다 많이 보내 남은 잔돈(5,000원 낼 것에 6,000원 → 1,000원)을 회원 본인에게 알리고 **환불받을 계좌번호를 청한다**. 종전엔 그 사실이 운영진 정산함에만 보였고, 보내려면 회원 계좌가 필요한데 물어볼 계기가 없어 잔돈이 묶여 있었다.
  - 근거 = RPC **`dues_my_refund_pending()`**(SECURITY DEFINER — `bank_transactions` 는 운영진 전용): 내가 낸 입금(`paid_by` 또는 내 배분이 붙은 입금) 중 `입금액 − 배분 − 환불 > 0` 인 것. **배분이 0인 입금은 제외**(정산 전이라 잔돈이 아니라 처리 대기), **묶음·분류·세션이 붙은 입금도 제외**(그 잔액은 의도된 클럽 수입 — 예: 54,000 중 30,000 회식 부과 + 24,000 콕 공동구매 모금).
  - 화면 = `RefundPendingCard` 한 컴포넌트를 이 탭과 진입 알림(§3.5)이 **같이 쓴다**(문구가 갈리면 어느 안내가 맞는지 묻게 된다). 미납(빨강)과 **반대 방향의 돈**이라 초록. 근거 줄(`8. 23. 6,000원 보냄 · 5,000원 사용 · 1,000원 남음`)을 함께 적어 "왜 남았는지"를 묻지 않게 한다.
...
### 3.5 회비 진입 알림 (앱 전역 모달, `UnpaidDuesAlert`)

미납자가 앱을 열면 **낼 금액·항목·입금 계좌**를 모달로 먼저 보여준다(2026-08-08). **돌려받을 돈**(§3.4)도 같이 알린다(2026-08-24) — 회원이 [내 회비]를 스스로 열지 않으면 잔돈이 있다는 걸 서로 모르고, 계좌번호가 없으면 운영진도 못 보낸다. 조회는 미납 확인과 **같은 1회 호출에 얹어** 진입 비용을 늘리지 않는다.

- **노출 조건**(전부 AND): 로그인 회원 · **프로필 완성**(미완성이면 Home 의 `ProfileSetup` 모달이 먼저 → 겹침 방지) · **(미납 잔액 > 0 또는 돌려받을 돈 > 0)**(§3.4 판정 공유) · 이번 앱 실행에서 안 닫음 · 보드(`/session`) 아님(경기 운영 화면을 가리지 않음).
...
- **낼 돈이 있으면 그게 주제다**(제목 `미납 …가 있어요`, 빨강 ⚠) — 돌려받을 돈은 그 아래에 덧붙는다. **돌려받을 돈만 있으면** 제목·아이콘이 바뀌고(`돌려받을 돈이 있어요`, 초록 ✓) **입금 계좌 블록은 감춘다**(보낼 곳이 아니라 받을 계좌가 필요하다).
...
- **조회**: `duesActions.checkUnpaidAlert(memberId)` — memberId 확정 시 1회(`unpaidAlertCheckedFor` 가드), `fetchMyCharges` + `fetchClubAccount` + `fetchMyRefundPending` 병렬. `/my-dues`(`loadMine`)와 **별도 스토어 슬라이스**(`unpaidAlert*`)라 두 화면의 로드가 서로를 덮지 않는다. 로그아웃/계정 전환 시 `resetUnpaidAlert`로 스냅샷 폐기.
...
- **세션 종료 최초 발행의 선납은 `new_members`가 아니다**(2026-08-31): 종료 트리거만 `dues_generate_session_court(session, true)`로 호출해, 기존 선납자가 있어도 나머지 정상 대상을 자동 발행한다. 종료 후 재계산(`p_initial=false`)에서 새로 붙은 대상만 `new_members` 대기로 보낸다. 실측 #165는 선납 12명을 "이미 발행"으로 보고 나머지 16명을 전부 대기로 보낸 것이 원인이었다(그 뒤 4명 즉석 발행 → 화면 `확인 12`).
...
- **대관비**: **세션 종료(closed)** 시 트리거 `trg_session_court_on_close` → 최초 초안 계산(`p_initial=true`) → 선납을 제외한 나머지는 즉시 발행, 금액 이상이면 발행 대기(§4.0). 종료 후 재계산에서 새 대상이 생긴 경우만 `new_members` 대기.
...
**열린 구멍(회비에 직접 영향, 미착수)**
- ~~**회원이 자기 가입일(`membership_started_at`·`created_at`)을 직접 바꿀 수 있다**~~ → **막았다** (2026-09-03, `20260903000000_newbie_freepass_capacity.sql`). 가입일이 회비 부과 시작월을 정하는데(§2) 본인 행 UPDATE 가 허용돼 있어 미래로 밀면 영구 면제였다. `trg_members_guard_join_date`(UPDATE 는 거부, INSERT 는 값 정리)로 닫았다 — `is_active` 게이트와 같은 패턴. **신규회원 2주 프리패스가 같은 값을 쓰게 되면서 발견**(참석 쪽 사양은 docs/EXPANSION_SPEC.md §5.1).
- **정원 초과 확정자(프리패스)가 회식비·수동부과 엔빵 분모에 들어간다.** 대관비는 운영진·신규 프리패스가 **부과 없는 장소에서만** 열리므로 영향이 0원이지만(`charges_court_fee=false` → `dues_generate_session_court` 가 0 반환), 무부과 장소의 정모도 수동부과 대상이라(§1.1) 회식비 인당이 그만큼 내려갈 수 있다. 이미 낸 사람은 옛 금액이 유지되므로(`amount_paid > 0` 은 자동정리 제외) 한 묶음에 두 금액이 남는다 — 잔돈은 §5 돌려받을 돈으로 보인다.
  - **우선참여권(대기 포인트 티켓, 2026-09-04 `20260904000000`)은 이 게이트 밖이다** — 부과 있는 일정에서도 정원 외 확정이 회차당 최대 2명 생긴다. 다만 **대관비는 정상 부과된다**: `dues_court_targets` 는 `capacity_exempt`/`exempt_reason` 을 보지 않고 `status in ('confirmed','late_pool')` 만 보므로 티켓 자리가 코드 변경 없이 대상에 들어간다(엔빵 분모에도 들어가고 인당도 그만큼 내려간다 — 참석했으니 의도된 동작이다). 티켓 사용자가 **당일 취소**하면 종전 규칙대로 정액 6,000원 부과 대상이 되고, 포인트는 몰수 + −1 이다.
