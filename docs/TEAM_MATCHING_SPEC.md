...
> **2026-09-13 임시 비활성화.** 보드 가운데 홀드 버튼과 안내를 숨기고, 패널의 상태 조회·참여·자동 확정도 실행하지 않는다. `SessionBoard.tsx`의 `MEMBER_PARTY_ENABLED = false`로 제어한다. 재개 시 `true`로 바꾸고 프론트를 배포한다. 아래는 재개 시 적용되는 기능 규칙이며 서버 RPC와 기존 확정 팀은 유지한다.
