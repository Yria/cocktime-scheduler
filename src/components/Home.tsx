...
export default function Home({ onStart }: Props) {
	...
	// 모임 장소 → 지도 열기 타깃(웹+앱 스킴). 있으면 장소명 탭 시 카카오맵(모바일=네이티브 앱) 열림.
	const placeMapTarget = (id: number | null) =>
		id == null ? null : buildPlaceMapTarget(places.find((p) => p.id === id) ?? null);
	...
	return (
		...
		<AppScreen
			...
			center={
				...
				showTicketBadge ? <HeaderTicketBadge /> : undefined
			}
			right={
					...
					<div className="flex flex-col gap-2.5">
						{orderedSchedules.map((s) => (
							<ScheduleCard
								...
								placeMapTarget={placeMapTarget(s.place_id)}
