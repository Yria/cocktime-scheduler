...
import {
	duesSetHonorary,
	fetchHonoraryReasons,
} from "../../lib/supabase/duesSettings";
import {
...
export default function HonoraryMembersSection() {
	...
	const refreshMembers = async () =>
		setMembers(await fetchMembersForAdmin(true));

	const [query, setQuery] = useState("");
	const [selected, setSelected] = useState<AdminMemberRow | null>(null);
	const [reason, setReason] = useState("");
	const [busyId, setBusyId] = useState<string | null>(null);
	const [confirmRemove, setConfirmRemove] = useState<AdminMemberRow | null>(
		null,
	);
	...
	useEffect(() => {
		let active = true;
		void fetchMembersForAdmin(true).then((rows) => {
