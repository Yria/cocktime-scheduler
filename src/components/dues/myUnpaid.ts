...
/**
 * 미납/부분납 부과만. 대관비는 월 무관 전부, 회비는 실효 월(이월=deferredTo, 없으면 periodYm)이
 * 기준 월 이하인 것만(미래로 이월된 건 아직 안 뜸). 회비 시스템 정산은 2026-07부터라 과거 회비 미납은 없음.
 * 면제(waived)·부과삭제(void)·완납은 status 필터에서 자연히 빠진다.
 */
...
/** '7월 회비' / '7.12 대관비' / '7.12 대관비 (게스트 대납)'. */
