import type { Player, PlayerSkills, SkillLevel, Gender } from '../types'
...
export async function fetchPlayers(spreadsheetId: string, apiKey: string): Promise<Player[]> {
...
export async function updatePlayerWithToken(
  ...
  const values = [
    playerName, gender,
    skills['클리어'], skills['스매시'], skills['로테이션'],
    skills['드랍'], skills['헤어핀'], skills['드라이브'], skills['백핸드'],
  ]
