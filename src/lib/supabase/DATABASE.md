...
- **획득**: (a) **진입 1회 자동 점유** — 세션 opener가 자유 보드에 들어오면 `useSessionBoardEffects`의 one-shot(`autoClaimTriedRef`)이 자유+혼자+운영진일 때 `claimEditingIfFree`로 점유. **"진입 1회만"**이라 이후 편집자 이탈로 free가 돼도 **재점유 안 함**(연속 재점유 `maybeClaimIfAlone`·reeval·창복귀 재점유는 폐기 → 호깅/플래핑 방지). ⚠️ 이 진입 자동점유를 없애면 opener가 콕체크 등 편집 불가(편집이 전부 isEditor 게이팅) — 실제 회귀 발생해 복구함. (b) **편집 동작 self-claim** — 드래그 편집(`claimEdit`→`claimEditingIfFree`, 자유 락만), 서버는 `board_save_drafts`/`board_assert_editor`가 확정. (c) **"편집 권한 가져오기"**(`claimEditor`→`board_takeover_editor`, 활성 보유자도 무조건 탈취). 탈취당한 보유자는 실시간 row 수신으로 보기 전용 수렴 + `EditorTakenNotice` 다이얼로그.
