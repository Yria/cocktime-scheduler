...
- **보유자** = `editor_client_id != null`(**신원만 — sticky**). `computeLockFromRow`로 `isEditor`/`holderClientId`/`holderName`/`lockFree` 산정. **2026-07 변경**: 하트비트·lease 만료 폐기 → 락은 만료로 자동 해제되지 않고 명시 이동으로만. `editor_lease_until`은 잔존하나 판정 미사용(마이그레이션 `20260717000000`).
- **획득**: (a) **진입 1회 자동 점유** — 세션 opener가 자유 보드에 들어오면 `useSessionBoardEffects`의 one-shot(`autoClaimTriedRef`)이 자유+혼자+운영진일 때 `claimEditingIfFree`로 점유. **"진입 1회만"**이라 이후 편집자 이탈로 free가 돼도 **재점유 안 함**(연속 재점유 `maybeClaimIfAlone`·reeval·창복귀 재점유는 폐기 → 호깅/플래핑 방지). ⚠️ 이 진입 자동점유를 없애면 opener가 콕체크 등 편집 불가(편집이 전부 isEditor 게이팅) — 실제 회귀 발생해 복구함. (b) **"편집 권한 가져오기"**(`claimEditor`→`board_takeover_editor`). 자유(lockFree)면 다이얼로그 없이 즉시 점유, 활성 보유자면 확인 후 무조건 탈취(탈취당한 보유자는 실시간 row 수신으로 보기 전용 수렴 + `EditorTakenNotice` 다이얼로그). **드래그 편집 self-claim은 폐기**(2026-07-22) — `claimEdit`는 이제 `isEditor`만 반환한다. 자유 상태여도 드래그로 암묵 점유하지 않으므로, 편집자가 이탈해 free가 돼도 관전자는 계속 보기 전용이고 버튼으로만 편집권을 얻는다.
...
## 운영진 교대 필수 조건 (2026-09-20)

참가해 도착이 확인된 운영진 중 최소 한 명은 경기 밖에 남아야 한다. 휴식 중인 운영진은
경기 밖 인원으로 인정하며, 콕 체크를 사용하는 세션에서 미확인 등록자는 인정하지 않는다.
마지막 운영진이 포함된 팀은 대체 팀 유무와 관계없이 교대까지 대기한다. 예외 시작은 제공하지 않는다.

`20260920020000_require_admin_match_coverage.sql`은 기존 예외 인자를 무시하고, `matches`의
경기 시작·선수 교체를 세션 행 잠금 안에서 검사한다. 이전 경기 시작 RPC와 직접 쓰기도
같은 검사를 거친다. 진행 중 운영진과 대기 운영진의 맞교대, 경기 종료는 허용한다.
이미 운영진이 모두 경기 중이거나 참석하지 않은 경우 일반 회원의 경기를 막지는 않는다.
