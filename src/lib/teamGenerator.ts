 ...
 */
import type { Player, GeneratedTeam, GameType, PairHistory, MixedHistory, GameCountHistory, SkillLevel } from '../types'
...
function pairingScore(
  ...
  const sA0 = skillScore(teamA[0]), sA1 = skillScore(teamA[1])
  const sB0 = skillScore(teamB[0]), sB1 = skillScore(teamB[1])
  ...
  // 파트너 중복 페널티 (최우선)
  const historyPenalty = partnerCount(teamA[0], teamA[1], history)
...
function determineGameType(players: Player[], singleWomanIds: string[]): GameType {
  const women = players.filter(p => p.gender === 'F')
  if (women.length === 0) return '남복'
  if (women.length === 4) return '여복'
  if (women.length === 2) return '혼복'
  // 여자 1명 또는 3명
  if (women.length === 1) return singleWomanIds.includes(women[0].id) ? '혼합' : '남복'
  return '남복'
}
...
function selectFour(
  ...
  if (women.length === 1 && !singleWomanIds.includes(women[0].id) && men.length >= 4) {
...
export function generateTeam(
  ...
  const selected = selectFour(waiting, mixedHistory, gameCountHistory, singleWomanIds)
