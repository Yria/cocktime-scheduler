...
export function cardControls(showUnconfirm: boolean, proposal = false) {
	const left = -TEAM_W / 2 + TEAM_PAD;
	const dismissWidth = proposal ? 44 : UNCONFIRM_W;
	const offset = showUnconfirm ? dismissWidth + UNCONFIRM_GAP : 0;
	return {
		main: { x: left + offset, y: CARD_CTA_Y, width: TEAM_W - TEAM_PAD * 2 - offset, height: TEAM_CTA_H },
		unconfirm: showUnconfirm ? { x: left, y: CARD_CTA_Y, width: dismissWidth, height: TEAM_CTA_H } : null,
	};
}
