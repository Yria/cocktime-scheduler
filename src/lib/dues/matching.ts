...
//   · 비활성 회원 제외 — 비활성이 되면 회비 미납은 면제되지만 **납부분·대관비 부과는 그대로 남고**,
//     뒤늦게 내는 입금도 들어온다(실측: 이한비 님 2026-07). 붙일 사람이 후보에서 사라지면 배분할 길이
//     없으므로 빼지 않고 동명 그룹 안에서만 뒤로 보낸다.
 ...
 */
export function sanitizeCandidatePool<T extends CandidatePoolMember>(
	members: T[],
	...
	for (const [key, g] of groups) {
		...
		const ordered: T[] = [
			...real.filter((m) => m.isActive !== false),
			...real.filter((m) => m.isActive === false),
		];
		const dropGuests = real.length > 0 && !opts.keepGuestsWithNamesakeMember;
		const guestOrdered = guestsRecentFirst(guests);
		const protectedGuests = protectedIds
			? guestOrdered.filter((gu) => protectedIds.has(gu.id))
			: [];
		if (protectedGuests.length > 0) {
			...
			if (
				opts.keepGuestsWithNamesakeMember &&
				guestOrdered.length > 0 &&
				!protectedGuests.includes(guestOrdered[0])
			) {
 ...
 */
export function suggestMembers<T extends MemberLite>(
	rawName: string,
	members: T[],
	opts?: CandidatePoolOptions,
): (T & { score: number })[] {
