...
describe("splitAmount — 총액 엔빵", () => {
	const split = (
		total: number | null,
		head: number,
		unit: 10 | 100 | 1000 = 1000,
	) => splitAmount({ mode: "total", total, perHead: null, head, unit });

	it("딱 나눠지면 차액이 0", () => {
		expect(split(442000, 17, 10)).toEqual({
			perHead: 26000,
			head: 17,
			charged: 442000,
			diff: 0,
	...
	it("절상한다 — 총액보다 더 걷히고 그 차액을 돌려준다", () => {
		...
		expect(split(100000, 17)).toEqual({
			perHead: 6000,
			head: 17,
			charged: 102000,
			diff: 2000,
	...
	it("1명이면 총액을 그 사람이 다 낸다(절상 단위로 올림)", () => {
		expect(split(33500, 1, 1000)).toEqual({
			perHead: 34000,
			head: 1,
			charged: 34000,
			diff: 500,
