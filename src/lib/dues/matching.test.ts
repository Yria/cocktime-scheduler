...
const member = (
	id: string,
	name: string,
	extra: Partial<{ isActive: boolean }> = {},
) => ({
	id,
	name,
	gender: "M" as const,
...
describe("sanitizeCandidatePool", () => {
	it("동명 회원+게스트 → 회원만 남는다", () => {
		const r = sanitizeCandidatePool([
			guest("g1", "김지훈", "2026-07-01"),
			member("m1", "김지훈"),
			guest("g2", "김지훈", "2026-08-01"),
		]);
		expect(r.map((m) => m.id)).toEqual(["m1"]);
	});
	it("동명 게스트 3행 → 최신 1행", () => {
		const r = sanitizeCandidatePool([
			guest("g1", "문병기", "2026-06-01"),
			guest("g2", "문병기", "2026-08-05"),
			guest("g3", "문병기", "2026-07-03"),
		]);
	...
	it("미납이 걸린 게스트 행은 접지 않는다(배분 경로 보존)", () => {
		const r = sanitizeCandidatePool(
			[
				guest("g1", "김윤호", "2026-06-01"),
				guest("g2", "김윤호", "2026-08-01"),
				guest("g3", "김윤호", "2026-07-01"),
			],
			{ protectedIds: ["g3"] },
		);
	...
	it("검색 완화 옵션: 동명 회원이 있어도 게스트 대표 1행은 남는다", () => {
		const r = sanitizeCandidatePool(
			[
				member("m1", "김선예"),
				guest("g1", "김선예", "2026-06-01"),
				guest("g2", "김선예", "2026-08-01"),
			],
			{ keepGuestsWithNamesakeMember: true },
		);
	...
	it("동명이 아닌 회원의 상대 순서는 흔들지 않는다", () => {
		const pool = [
			...
			guest("g1", "문병기", "2026-06-01"),
			member("b", "장세훈"),
			guest("g2", "문병기", "2026-08-01"),
			member("c", "박세경"),
		];
		expect(sanitizeCandidatePool(pool).map((m) => m.id)).toEqual([
			"a",
			"g2",
			"b",
			"c",
...
describe("suggestMembers × 후보 위생", () => {
	...
	it("게스트만 있는 이름은 대표 1행이 후보로 남는다(게스트 대관비 입금 매칭 유지)", () => {
		const pool = [
			guest("g1", "고은림", "2026-06-01"),
			guest("g2", "고은림", "2026-08-01"),
		];
