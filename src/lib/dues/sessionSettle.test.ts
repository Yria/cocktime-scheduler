...
import type { AdminMemberRow } from "../supabase/adminMembers";
import type {
	CourtChargeRow,
...
const att = (
	memberId: string,
	status: string,
	over: Partial<SessionAttendanceRow> = {},
): SessionAttendanceRow => ({
	memberId,
	status,
	confirmedAt:
		status === "confirmed" || status === "late_pool" ? kst(10, 12) : null,
	cancelledAt: null,
	...over,
});
...
const dayCancel = (memberId: string): SessionAttendanceRow =>
	att(memberId, "cancelled", {
		confirmedAt: kst(12, 9),
		cancelledAt: kst(12, 12),
	});
...
const graceCancel = (memberId: string): SessionAttendanceRow =>
	att(memberId, "cancelled", {
		confirmedAt: kst(12, 9),
		cancelledAt: kst(12, 9, 0),
	});

const session = (
	attendances: SessionAttendanceRow[],
	over: Partial<SessionFeeRow> = {},
): SessionFeeRow => ({
	id: 1,
	title: null,
	status: "closed",
...
describe("isDayCancelChargeable — 당일취소 술어 미러", () => {
	...
	it("확정 후 5초 철회(grace 1시간 내) → 대상 아님 (김영주 2.7초 사고 회귀 방지)", () => {
		const a = att("a", "cancelled", {
			confirmedAt: kst(12, 9),
			cancelledAt: kst(12, 9),
		});
	...
	it("전날 취소(사전취소) → 대상 아님", () => {
		const a = att("a", "cancelled", {
			confirmedAt: kst(10, 12),
			cancelledAt: kst(11, 20),
		});
		expect(isDayCancelChargeable(a, SCHEDULED)).toBe(false);
	});
	...
	it("KST 자정 경계: 세션 당일 23:30 취소는 대상, 다음날 00:10 은 아님", () => {
		expect(
			isDayCancelChargeable(
				att("a", "cancelled", {
					confirmedAt: kst(12, 20),
					cancelledAt: kst(12, 23, 30),
				}),
				SCHEDULED,
			),
		).toBe(true);
...
describe("buildSessionSettle — 정액(6,000) 세션", () => {
	...
	it("8/30 #165 회귀: 대상 28·기발행 16·초안 16(4건 중복)은 발행대기 12, 확인 0", () => {
		const atts = Array.from({ length: 28 }, (_, i) =>
			att(`p${i}`, "confirmed"),
		);
		const charges = Array.from({ length: 16 }, (_, i) =>
			charge(i + 1, `p${i}`),
		);
		// 종료 때 생긴 초안 16명 중 p12~p15 네 명은 이후 입금 확인으로 즉석 발행된 상태.
		const draftIds = new Set(
			Array.from({ length: 16 }, (_, i) => `p${i + 12}`),
		);
		const settle = buildSessionSettle(
			session(atts),
			charges,
	...
	it("부과삭제(void)된 사람은 missing 이 아니다 — 의도된 면제이므로 재부과를 유도하지 않는다", () => {
		...
		expect(settle.charged.find((c) => c.name === "최서연")?.voidedByName).toBe(
			"운영진A",
		);
	});
	...
	it("게스트 대납은 대납자 이름을 명단에 붙인다", () => {
		const withGuest = dict(
			...[...members.values()],
			member("김하늘", { isGuest: true }),
			member("박지훈"),
		);
		const s = session([att("김하늘", "confirmed")]);
		const settle = buildSessionSettle(
			s,
			[charge(1, "김하늘", { payerHint: "박지훈" })],
			withGuest,
			6000,
			[],
		);
...
describe("buildSessionSettle — 엔빵 세션", () => {
	...
	it("10원 절상: 나누어떨어지지 않으면 올린다(총액보다 조금 더 걷힌다)", () => {
		const s = session(
			[att("A", "confirmed"), att("B", "confirmed"), att("C", "confirmed")],
			{ courtFee: 100000 },
		);
		const settle = buildSessionSettle(s, [], members, 6000, []);
	...
	it("정액 근처 스냅: 정액 이상 +200원 미만은 정액으로, 정액보다 싸면 계산값 그대로", () => {
		...
		const snapped = buildSessionSettle(
			session(many, { courtFee: 117000 }),
			[],
			dict(),
			6000,
			[],
		...
		const outside = buildSessionSettle(
			session(
				Array.from({ length: 10 }, (_, i) => att(`Q${i}`, "confirmed")),
				{ courtFee: 63000 },
			),
			[],
			dict(),
		...
		const cheap = buildSessionSettle(
			session(
				Array.from({ length: 10 }, (_, i) => att(`R${i}`, "confirmed")),
				{ courtFee: 58500 },
			),
			[],
			dict(),
	...
	it("당일취소인데 보드에 올라가 실제로 뛰었으면 당일취소로 세지 않는다", () => {
		const s = session([att("A", "confirmed"), dayCancel("C")], {
			courtFee: 90000,
			boardMemberIds: ["A", "C"],
		});
		const settle = buildSessionSettle(s, [], members, 6000, []);
	...
	it("참석 기록이 아예 없는 부과(선납 후 세션 이탈)는 noAttendance 로 표시한다", () => {
		...
		const settle = buildSessionSettle(
			s,
			[charge(1, "A"), charge(2, "B", { amountPaid: 6000, status: "paid" })],
			members,
			6000,
			[],
		);
...
describe("buildSessionSettle — 안 걷는 회차(총액 0 이하)", () => {
	...
	it("참석자는 '안 걷는 회차'로 면제 명단에 뜬다(운영진도 같은 사유)", () => {
		const s = session([att("A", "confirmed"), att("운영진A", "confirmed")], {
			courtFee: 0,
		});
		const settle = buildSessionSettle(s, [], members, 6000, []);
		expect(settle.exempt.map((e) => [e.name, e.reason])).toEqual([
	...
	it("무부과 회차에 남은 부과는 잔재(stale)로 세운다 — 정리 대상임을 화면이 말한다", () => {
		...
		expect(
			settle.roster.filter((r) => r.kind === "stale").map((r) => r.name),
		).toEqual(["A", "B"]);
	});
...
describe("courtPerHead — 서버 dues_court_per_head 미러", () => {
	...
	it("당일취소·보드 추가분도 분모에 든다(서버 대상 술어와 동일)", () => {
		const s = session([att("A", "confirmed"), dayCancel("B")], {
			courtFee: 40000,
			boardMemberIds: ["C"],
		});
	...
	it("총액 0 이하면 null — 안 걷는 회차라 칩을 감춘다", () => {
		expect(
			courtPerHead(session([att("A", "confirmed")], { courtFee: 0 }), 6000),
		).toBeNull();
		expect(
			courtPerHead(session([att("A", "confirmed")], { courtFee: -5000 }), 6000),
		).toBeNull();
	});
	...
	it("정액 근처 스냅도 같이 적용된다(한방향)", () => {
		...
		expect(courtPerHead(session(nineteen, { courtFee: 117000 }), 6000)).toBe(
			6000,
		);
	});
});
...
function expectIdentities(settle: ReturnType<typeof buildSessionSettle>) {
	const bridge =
		settle.mode === "none"
			...
			: (settle.mode === "split"
					? settle.attendCount
					: settle.attendCount - settle.adminAttendCount) +
				settle.targetDayCancelCount +
				settle.boardAddedCount;
	expect(bridge).toBe(settle.targetCount);
	expect(
		settle.targetCount -
			settle.missing.length -
			settle.pending.length -
			settle.deadOnTargetCount +
			settle.liveExtraCount,
	).toBe(settle.activeCount);
}
...
describe("buildSessionSettle — 통합 명단(roster)", () => {
	...
	it("부과 있는 사람 + 누락 + 정상 면제를 모두 담는다(빠지는 사람 없음)", () => {
		...
		expect(settle.roster.length).toBe(
			settle.charged.length +
				settle.missing.length +
				settle.pending.length +
				settle.exempt.length,
		);
	});
	...
	it("사전취소·대기한 운영진은 '운영진 면제'로 올라오지 않는다(오지도 않은 사람이 면제로 보임 방지)", () => {
		...
		expect(s.exempt.map((e) => [e.name, e.reason])).toEqual([
			["운영진A", "adminFlat"],
		]);
	...
	it("헤더 카운트는 서로 겹치지 않는다 — 합 + 확인필요 = 명단 수", () => {
		...
		expect(paid + unpaid + dead + none + pending + settle.flaggedCount).toBe(
			settle.roster.length,
		);
	...
	it("확인 필요 건수는 카드 ⚠배지와 같은 값이고 고아 부과는 포함하지 않는다", () => {
		expect(settle.flaggedCount).toBe(
			settle.missing.length + settle.staleCharges.length,
		);
	...
	it("void 된 당일취소는 stale 이 아니라 charged 로 남는다(이미 처리된 건은 확인 대상 아님)", () => {
		...
		const row = s2.roster.find(
			(r) => r.name === "즉시철회" && r.charge != null,
		)!;
...
describe("buildSessionSettle — 확인 대상 좁히기", () => {
	...
	it("참석 기록이 없는 부과는 orphan 으로 빼서 확인 대상에서 제외한다 (7/5 #166 실측)", () => {
		...
		const settle = buildSessionSettle(
			...
			[
				charge(1, "A"),
				charge(2, "옛회원", { amountPaid: 6000, status: "paid" }),
			],
			members,
			6000,
			[],
	...
	it("살아 있고 참석 기록도 있는 위반만 확인 대상 — 사전취소 완납 건", () => {
		...
		const settle = buildSessionSettle(
			...
			[
				charge(1, "A"),
				charge(2, "차성민", { amountPaid: 6000, status: "paid" }),
			],
			members,
			6000,
			[],
		);
		expect(settle.staleCharges.map((c) => [c.name, c.extraReason])).toEqual([
			["차성민", "preCancel"],
		]);
	});
});
...
describe("buildSessionSettle — 실측 회귀(에이트민턴 8/9·8/2)", () => {
	it("8/9 #106: 참석 23명인데 낼 돈 156,000 — 운영진 −1, 당일취소 +3, 사전취소 잔재 +1", () => {
		...
		for (let i = 0; i < 22; i++) {
			...
			charges.push(
				charge(100 + i, `p${i}`, { amountPaid: 6000, status: "paid" }),
			);
		}
		...
		const settle = buildSessionSettle(session(atts), charges, members, 6000, [
			{ direction: "out", amount: 195000 },
		]);
