import { nameWithBirthYear } from "../birthYear";
...
function snapToFlat(perHead: number, flatFee: number): number {
	return perHead >= flatFee && perHead < flatFee + FLAT_SNAP_BAND
		? flatFee
		: perHead;
}
 ...
 */
export function isDayCancelChargeable(
	a: SessionAttendanceRow,
	...
	return (
		new Date(a.cancelledAt).getTime() - new Date(a.confirmedAt).getTime() >=
		GRACE_MS
	);
}

/** 당일 취소이긴 하나 grace(1시간) 안에 스스로 물린 건 — 부과 제외 사유로 명시 노출한다. */
function isGraceWithdrawn(
	a: SessionAttendanceRow,
	scheduledAt: string | null,
): boolean {
	if (
...
const CALM_REASONS = new Set<NonTargetReason>([
	"adminFlat",
	"grace",
	"noCourtFee",
]);
...
const KIND_RANK: Record<RosterKind, number> = {
	missing: 0,
	stale: 1,
	pending: 2,
	charged: 3,
...
export function buildSessionSettle(
	...
	memberById: ReadonlyMap<
		string,
		Pick<AdminMemberRow, "id" | "name" | "birthYear" | "isAdmin" | "isGuest">
	>,
	flatFee: number,
	...
	const perHead = noCharge
		...
		: split
			? splitHead > 0
				? snapToFlat(
						Math.ceil((total as number) / splitHead / 10) * 10,
						flatFee,
					)
				: 0
			: flatFee;
	...
	const reasonCount = (reason: NonTargetReason) =>
		[...nonTarget.values()].filter((r) => r === reason).length;
	...
	const exempt: SettleExemptRow[] = [...nonTarget.entries()]
		.filter(
			([memberId, reason]) =>
				CALM_REASONS.has(reason) && !chargedIds.has(memberId),
		)
		.map(([memberId, reason]) => {
	...
	const statusRank = (s: ChargeStatus) =>
		s === "paid" || s === "overpaid"
			? 0
			: s === "waived" || s === "void"
				? 2
	...
	const charged: SettleChargeRow[] = charges
		.map((c) => {
			const proxy =
				c.payerHint && c.payerHint !== c.memberId ? c.payerHint : null;
			return {
				chargeId: c.id,
				memberId: c.memberId,
				...
				payerName: proxy
					? nameWithBirthYear(nameOf(proxy), birthYearOf(proxy))
					: null,
				voidedByName: c.voidedBy
					? nameWithBirthYear(nameOf(c.voidedBy), birthYearOf(c.voidedBy))
					: null,
				isAdmin: isAdminOf(c.memberId),
				live: c.status !== "void" && c.status !== "waived",
				extraReason: targets.has(c.memberId)
					? null
					: (nonTarget.get(c.memberId) ?? "noAttendance"),
			};
		})
		.sort(
			(a, b) =>
	...
	const pending = unissued.filter(
		(m) => session.status !== "closed" || draftMemberIds.has(m.memberId),
	);
	const missing = unissued.filter(
		(m) => session.status === "closed" && !draftMemberIds.has(m.memberId),
	);
	...
	const exemptSorted = exempt.sort(
	...
	const roster: SettleRosterRow[] = [
		...missing.map(
			(m): SettleRosterRow => ({
				key: `m${m.memberId}`,
				name: m.name,
				birthYear: birthYearOf(m.memberId),
				kind: "missing",
				charge: null,
				reason: null,
				targetReason: m.targetReason,
				isAdmin: m.isAdmin,
			}),
		),
		...pending.map(
		...
		...exemptSorted.map(
			(e): SettleRosterRow => ({
				key: `e${e.memberId}`,
				name: e.name,
				birthYear: birthYearOf(e.memberId),
				kind: "exempt",
				charge: null,
				reason: e.reason,
				targetReason: null,
	...
	for (const r of roster) {
		if (r.kind === "missing" || r.kind === "stale")
			flaggedCount++; // 납부 상태보다 우선해 센다
	...
	return {
		...
		perHead,
		dueAmounts: [...new Set(live.map((c) => c.amountDue))].sort(
			(a, b) => a - b,
		),
		...
		missing,
		pending,
		deadOnTargetCount: charged.filter((c) => !c.live && c.extraReason == null)
			.length,
		liveExtraCount: charged.filter((c) => c.live && c.extraReason != null)
			.length,
		...
		exempt: exemptSorted,
		extra,
		staleCharges: extra.filter(
			(c) => c.live && c.extraReason !== "noAttendance",
		),
