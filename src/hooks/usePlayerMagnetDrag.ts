...
export function usePlayerMagnetDrag({
	...
	const handleDragStart = useCallback(
		(e: Konva.KonvaEventObject<DragEvent>) => {
			...
			// 드래그 정보 등록 — 팀 소속(anchor/ghost)이면 상단 '팀에서 빼기', restable이면 하단 휴식존 노출.
			const teamBound = isGhost || !!store.magnets.get(playerId)?.teamId;
			// 하단 휴식존 드롭 가능: 편집자의 free/anchor 자석(예약·경기중 제외). 휴식자도 포함 — 같은 존에
			// 다시 놓아 복귀시키는 토글이라 진입/해제가 대칭이다.
			const restable = useSessionStore.getState().isEditor && !isGhost && !playing;
			store.setDragInfo({ playerId, detachable: teamBound, restable });
		...
		[clearTap, clearLongPress, isGhost, playing, playerId],
	);
	...
	const handleDragEnd = useCallback(
		(e: Konva.KonvaEventObject<DragEvent>) => {
			...
			// 슬롯 복귀: ghost/playing이거나, 드롭 후에도 여전히 팀 anchor면 슬롯(offset)으로.
			// (휴식 자석은 자유 자석과 동일 — 딱지만 달고 보드에 남아 자유롭게 위치 이동한다.)
			...
			// 위 드롭 콜백들이 dragInfo(restable 등)를 읽으므로 반드시 그 "이후"에 호출한다.
			useBoardStore.getState().clearDrag();
		},
		[playerId, isGhost, playing, reservationId, onDragEnd, onGhostDragEnd, onPlayingDragEnd, offsetX, offsetY, cancelDragRaf, justDraggedRef],
	);
